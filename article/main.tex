...
\usepackage{graphicx}
% ---------------------------------------------------------------------------
% LISIBILITE — voir docs/articles/STYLE_ARTICLES.md §2. Mesure : a 11pt, une
% marge de 2,7cm donne une ligne de 15,6cm, soit ~90 caracteres, tres au-dessus
% de l'optimum de lecture (65-75) ; et \parskip nul ne separe pas visuellement
% deux paragraphes. Les deux ensemble font la page en « bloc gris », quelle que
% soit la longueur des paragraphes (mesuree ici : mediane 652 caracteres).
% ---------------------------------------------------------------------------
...
\begin{enumerate}
  ...
  \item Certified failure objects with computed scope, and the E1--E7 taxonomy
        (seven classes identified, each instantiated; the list is open),
...
four logical signs $\square$ (which marks a position a bound variable used to
...
\noindent
\textbf{These eight are the constructors of the ABBREVIATED layer}, the one the kernel
manipulates --- and they must be distinguished from Bourbaki's primitives, on pain of
drawing the boundary in the wrong place. The book's logical signs are \emph{four}:
$\square$, $\tau$, $\vee$, $\neg$. In the list above:

\begin{itemize}\itemsep1pt
\item $(\exists x)R$ \textbf{is already an abbreviation} in Bourbaki's sense --- it is
  $(\tau_x(R) \mid x)\,R$ (Section~\ref{sec:tau}). The kernel gives it a node of its own
  because unfolding would mean manipulating assemblies of astronomical size;
\item $=$ and $\in$ are not \emph{logical} signs but \emph{specific} ones, relational of
  weight~2: $=$ belongs to egalitarian theories (E~I.38), $\in$ to the theory of sets.
  They are therefore a special case of $s(T_1,\dots,T_n)$, singled out here only because
  the kernel treats them separately;
\item $\forall$, $\wedge$, $\Rightarrow$, $\Leftrightarrow$ are abbreviations to which
  the kernel gives no node at all.
\end{itemize}

\noindent
The list describes a \emph{representation}, not primitiveness. Writing a sign commits one
to the theory that owns it, and this definition borrows two; left unsaid, the abbreviated
layer would read as if it were Bourbaki's language.
...
records is that the hypothesis has moved from $H(\theta)$ into $C(\theta)$. That move is the whole of \emph{discharge}, and Section~\ref{sec:certified-failure}
...
The corpus at the time of writing: 4{,}221 collected tests re-deriving its theorems at
...
\paragraph{A third reading, and the one that matters most: this table does not say the
book is done.} It counts what is \emph{formalized and anchored}, not what the book
contains. The formalization is partial and uneven, and two figures say so better than a
caveat. First, among notions of a kind that \emph{promises a proof} --- proposition,
theorem, corollary, criterion, lemma --- 815 are adjudicated by the kernel and
\textbf{only 413 are closed, that is 50.7\%} (measured 18~August 2026); the rest are
proved under honestly declared hypotheses, or not yet. Second, major results of the book
remain open in the corpus: Hessenberg's theorem, the well-ordering of cardinals,
Cantor's theorem, Euclidean division, and projective and inductive limits.

We insist because the table invites exactly the opposite reading: one sees ``\S I.1: 82''
and understands ``\S I.1, done''. The corpus is large enough to carry this paper's
results, and it is not the book. This is the same defect Section~\ref{sec:fidelity}
treats, applied to a table rather than to a statement: nothing in the repository prevents
an exact number from suggesting a false thing.
...
The negation $\Gamma \nvdash R$ is not symmetric with it, and the asymmetry matters
throughout. $\vdash$ is established by exhibiting one object; $\nvdash$ asserts that
\emph{no} object exists, which the kernel cannot witness. Wherever $\nvdash$ appears
below it is therefore either a proved refutation ($\Gamma \vdash \neg R$, a stronger
claim) or a \emph{declared default} after a bounded search --- never a theorem. The
trichotomy of Section~\ref{sec:trichotomy} is built on exactly that distinction, and
$\vdash$ and $\nvdash$ are metalinguistic notations about the kernel, not signs of any
theory it manipulates.

A \emph{goal} is a pair $(T, P)$ with $T$ a theory and $P \in \mathcal{R}$ the relation
to be derived in it; we write the goal as $P$ alone whenever the theory is fixed by
context, which it is throughout this paper unless stated. A \emph{route} to a goal is
one attempted derivation of $P$, and the \emph{residue} of a route is the set of
hypotheses the theorem it reaches still carries --- that is, $H(\theta)$ for the
$\theta$ the route produced. A goal typically has several routes,
...
match the code. Here $P$ is the \emph{goal} in the sense fixed above --- a relation of
$\mathcal{R}$ to be derived in a theory --- not the route and not the theorem it
reached; $\kappa \in \mathcal{K}$ its failure class, where $\mathcal{K}$ is the set of
classes we have \emph{identified} and $|\mathcal{K}| \ge 7$ (see below); $c$ a kernel
object certifying that class,
...
%% ---- the goal, on top: what we WANT
\node[goal, draw=gray!70, fill=gray!8] (P)
  {$(n{+}1)! = n!\cdot(n{+}1)$ \;\textbf{?}};
\node[right=2mm of P, font=\scriptsize\itshape, gray!55!black] {the goal};

%% ---- the wall, with what it MEANS written on it
\node[below=3.5mm of P] (murbar) {\textcolor{gray!55!black}{\rule{74mm}{1.5mm}}};
\node[right=2mm of murbar, font=\scriptsize\itshape, gray!55!black, align=left,
      text width=27mm] {the \textbf{wall}: no amount of proof search crosses it};

%% ---- the four hypotheses left open (one per row of Table 1)
\node[open, draw=green!50!black, below=5mm of murbar, xshift=-27mm] (bo)
  {$bo(\le,\mathbb{N})$};
\node[open, draw=red!70!black, right=of bo] (hg) {$\mathcal{H}$};
\node[open, draw=green!50!black, right=of hg] (h23) {$H_2,\,H_3$};
\node[open, draw=orange!85!black, right=of h23] (hw) {$H_W,\,H_N$};
\node[left=2mm of bo, font=\scriptsize\itshape, gray!55!black, align=right,
      text width=15mm] {left \mbox{\textbf{open}}};

%% ---- what is ESTABLISHED, at the bottom
\node[th, below=6mm of hg, xshift=13mm] (chain)
  {C60/C62 chain \;+\; case-0 equations};
\node[ax, below=4mm of chain] (axs)
  {$A_{T_0}$ \;+\; dedicated theories $(Ax(D))$};
\node[left=2mm of axs, font=\scriptsize\itshape, gray!55!black, align=right,
      text width=15mm] {\textbf{established}};

\draw[fl] (axs) -- (chain);
\draw[fl, dashed] (chain) -- (bo);
\draw[fl, dashed] (chain) -- (hg);
\draw[fl, dashed] (chain) -- (h23);
\draw[fl, dashed] (chain) -- (hw);

%% ---- no « reading direction » arrow: it collided with the left-hand label,
%% and the caption already says « read from the bottom up ». A redundant cue
%% that wrecks the layout costs more than it gives.

%% ---- colour key, READ IN PLACE rather than in the table
\node[below=5mm of axs, font=\scriptsize, align=center] (leg)
  {\tikz\draw[draw=green!50!black,dashed,rounded corners=2pt](0,0)rectangle(3.4mm,2.2mm);
   \; dischargeable \quad
   \tikz\draw[draw=red!70!black,dashed,rounded corners=2pt](0,0)rectangle(3.4mm,2.2mm);
   \; refutable \quad
   \tikz\draw[draw=orange!85!black,dashed,rounded corners=2pt](0,0)rectangle(3.4mm,2.2mm);
   \; independent};
\end{tikzpicture}}
\caption{A real wall: the factorial front's last open equation. \textbf{Read from the
bottom up}: at the bottom what is established (solid boxes --- axioms, then kernel
theorems), above it the four hypotheses the derivation chain leaves \emph{open} (dashed
boxes), and the bar is the wall standing between them and the goal. The colour of an
open box is its trichotomy verdict, given in the key: only the orange one ---
\emph{independent} --- is a wall in the strict sense, the other two close. The four boxes
correspond one-to-one to the rows of Table~\ref{tab:frontier}.}
\label{fig:wall}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{The failure taxonomy \emph{as far as we have identified it}. Every class listed
is instantiated by a real, kernel-checked case from the instrumented week. The list is
\textbf{open}: nothing here shows that seven are all there are.}
\end{table}

\paragraph{The taxonomy is open, and cannot be closed by the same means as the rest.}
We write $\mathcal{K}$ for the set of failure classes and $|\mathcal{K}| \ge 7$ rather
than $\kappa \in \{E1,\dots,E7\}$, because the second notation asserts something we have
not established. Seven classes were identified and each was instantiated by a real case;
that a failure exists which fits none of them is neither excluded nor excludable here.
The classes were found by looking at failures, not derived from a principle, so the
catalogue grows by encounter: a failure matching no $\kappa$ becomes $E8$.

The asymmetry is the same one this paper meets twice elsewhere. Infidelity is
semi-decidable --- one can always \emph{find} one, never certify their absence
(Section~\ref{sec:fidelity}); novelty likewise (Section~\ref{sec:related}). Exhaustiveness
of $\mathcal{K}$ is of that family: exhibiting an eighth class is a finite act, proving
there is none is not. We therefore report a catalogue with a lower bound, not a type with
seven inhabitants, and we would rather the reader find an $E8$ than believe the list
closed because we typeset it with braces.
...
Python~3.13, pytest~9.0.3 with \texttt{pytest-xdist}, \texttt{PYTHONIOENCODING=utf-8},
suites sharded by zone. The test counts quoted in the case study above are those of the
instrumented week and are left as measured then; for the record, the suite re-run in
full on 20~August 2026 reports \textbf{4{,}221 passed in 2\,h\,03} at twelve workers
with \texttt{--dist loadfile}. We report both rather than retrofitting the narrative
with a later number. The debt instrument is valid
...
Two days after the week reported above (August 5--6), the same instrumentation was
turned toward an \emph{open} statement: the Goldbach conjecture, written over the
corpus's constructed arithmetic. We report here only what bears on this paper's
theses --- that statement defects are \emph{proved} and that debt is \emph{measured}.
The reductions the episode produced, and the measurement of how much arithmetic they
contain, are the subject of a companion paper; the discovery organs it exercised belong
to another still.

\emph{(i) Two statement defects, proved in the kernel.} The first rendering of the
conjecture read ``$n$ even and $n \neq 2$''. Its antecedent is satisfied at $n = 0$ ---
proved, closed: $\vdash \mathrm{even}(0)$ (witness $k := 0$) and $\vdash \neg(0 = 2)$ ---
so the statement asserted that \emph{zero is the sum of two primes}. Repaired, it was
still wrong: ``even'' constrains $n$ to be a \emph{cardinal}, not an integer, and the
antecedent is also satisfied by $n := \mathbb{N} + \mathbb{N}$, which is infinite ---
likewise proved, the equality with the old antecedent instantiated being checked formula
against formula. The same fault --- a quantifier ranging over sets while believed to
range over integers --- had been committed the day before on primality's $(\forall d)$:
it is a \emph{pattern}, now on record.

Notably, the \emph{proved} (bounded) variant of the statement carried the finiteness
guard from its first draft, because its proof demanded it; only the \emph{unproved}
statement had drifted. \textbf{Proving forces hypotheses to be named; stating forces
nothing.} We regard this as the most transferable observation of the episode, and it is
why the fidelity audit of Section~\ref{sec:fidelity} targets statements that no proof
is currently exercising.

\emph{(ii) Debt, in three numbers.} ``Zero residual hypotheses'' and ``the 22-axiom
invariant holds'' read together as ``depends only on the 22'' --- false in the measured
sense, since axioms enter through the $\mathrm{axiom}$ rule and vanish from the sequent
(Section~\ref{sec:certified-failure}). Measured in a fresh process on the bounded
$n$-form: \textbf{0} residual hypotheses; \textbf{14} of the 22 reference axioms
consumed; \textbf{53} foreign formulas --- one single rule, criterion C54 (the graph of a
term-defined function, a theorem in Bourbaki, a dedicated-theory axiom here),
instantiated 52 times, plus one Knaster--Tarski instance. An honest result is announced
with all three numbers; the third says what remains to be derived.

On the episode's main equivalence the same measurement returns \textbf{0}; \textbf{14};
\textbf{73} --- spread over \emph{21} dedicated theories, the well-ordering machinery
(Zorn, Zermelo, Bourbaki--Witt) entering through the ``a lower bound of a finite is
finite'' lemma. Removing a bound is paid for in well-ordering: the jump from 2 to 21
theories \emph{measures} what generality costs. The first measurement returned zero in
one second --- the theorem came out of a cache, and the instrument is blind to what is
not built: the fresh-process rule, written for other measurements, applies to itself.

\emph{What is deferred.} The episode ran four days and left the axiom count at 22. Its
mathematical output --- a chain of certified reductions collapsing the conjecture onto a
single object, and the measurement, by parameterizing the primality predicate, that
those reductions contain no arithmetic at all --- is reported separately, as is the work
of the discovery organs it exercised. None of it comes close to the conjecture. What
belongs here is the pair of results above: an open statement is an unusually good place
to catch a fidelity defect, precisely because nothing is proving it.
...
process (measured $-62\%$ undercount otherwise); the full suite runs in
2\,h\,03 at twelve workers (2\,h\,43 at the four used during the week reported here)
and the environment kills long runs near 50 minutes, which bounds experimental
turnaround and dictated the sharding discipline.
...
\node[ax, font=\tiny] at ($(T1.center)+(-15mm,-5mm)$) (a22) {22 axioms};
\node[draw=orange!85!black, fill=orange!14, rounded corners=2pt, font=\tiny] at ($(T1.center)+(11mm,-5mm)$) (delta) {$\delta$: fam$\leftrightarrow$value};
...
4{,}221-test corpus into $10^5$--$10^6$ certified (state, action, verdict) tuples
