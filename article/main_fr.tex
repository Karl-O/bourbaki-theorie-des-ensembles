...
\texttt{karl.olivet@gmail.com}. Code et corpus certifié~: [URL du dépôt et empreinte du
commit étiqueté au moment de la soumission].}}
...
%% ---- le but, en haut : ce qu'on VOUDRAIT
\node[goal, draw=gray!70, fill=gray!8] (P)
  {$(n{+}1)! = n!\cdot(n{+}1)$ \;\textbf{?}};
\node[right=2mm of P, font=\scriptsize\itshape, gray!55!black] {le but visé};

%% ---- le mur, avec ce qu'il SIGNIFIE ecrit dessus
...
\node[right=2mm of murbar, font=\scriptsize\itshape, gray!55!black, align=left,
      text width=27mm] {le \textbf{mur}~: aucune recherche de preuve ne le franchit};

%% ---- les quatre hypotheses laissees ouvertes (une par ligne du tableau 1)
...
\node[left=2mm of bo, font=\scriptsize\itshape, gray!55!black, align=right,
      text width=15mm] {restées \mbox{\textbf{ouvertes}}};

%% ---- ce qui est ACQUIS, en bas
\node[th, below=6mm of hg, xshift=13mm] (chain)
  {chaîne C60/C62 \;+\; équations du cas 0};
...
%% ---- pas de fleche « sens de lecture » : elle percutait l'etiquette de gauche,
%% et la legende dit deja « Se lit de bas en haut ». Un repere redondant qui
%% abime la mise en page coute plus qu'il ne rapporte.

%% ---- legende des couleurs, LUE SUR PLACE plutot qu'au tableau
\node[below=5mm of axs, font=\scriptsize, align=center] (leg)
  {\tikz\draw[draw=green!50!black,dashed,rounded corners=2pt](0,0)rectangle(3.4mm,2.2mm);
   ...
   \; indépendante};
\end{tikzpicture}}
\caption{Un mur réel~: la dernière équation ouverte du front factorielle. \textbf{Se lit
de bas en haut}~: en bas ce qui est acquis (boîtes pleines --- axiomes, puis théorèmes
du noyau), au-dessus les quatre hypothèses que la chaîne de dérivation laisse
\emph{ouvertes} (boîtes en tirets), et la barre est le mur qui les sépare du but. La
couleur d'une boîte ouverte est son verdict de trichotomie, rappelé en légende~: seule
l'orange --- \emph{indépendante} --- est un mur véritable, les deux autres se ferment.
Les quatre boîtes correspondent une à une aux lignes du tableau~\ref{tab:frontier}.}
\label{fig:wall}
\end{figure}
