...
page-et-ligne, sous lequel deux défauts de transcription d'axiome ont été détectés en
...
une succession finie de signes --- les quatre signes logiques $\square$, $\tau$,
...
$T, U \in \mathcal{T}$, $R, S \in \mathcal{R}$. Ces huit constructeurs sont les
seuls~: $\forall$, $\wedge$, $\Rightarrow$ et $\Leftrightarrow$ sont des abréviations
bâties sur eux, au sens de Bourbaki comme dans le code.
\end{definition}
...
enregistre, c'est que l'hypothèse est passée de la gauche du signe $\vdash$ dans la
conclusion. Ce déplacement est toute la \emph{décharge}, et la
...
Le corpus à l'heure où nous écrivons~: 4\,099 tests collectés re-dérivant à chaque
passage ses théorèmes, 2\,181 notions ancrées au livre imprimé à la granularité
page-et-ligne, et un journal de 232 événements d'échecs certifiés --- l'objet d'étude
de cet article.

Sa répartition mérite d'être dite, car le titre du livre invite à une image fausse.
Les 2\,181 notions ancrées se distribuent sur la tour et sur les chapitres qu'elle
porte~:
...
Une \emph{route} vers un but est une dérivation tentée de ce but, et le \emph{résidu}
d'une route est l'ensemble des hypothèses que porte encore le théorème auquel elle
aboutit --- c'est-à-dire $H(\theta)$ pour le $\theta$ que la route a produit. Un but a
...
code. Ici $P$ est le but qui était visé, $\kappa \in \{E1,\dots,E7\}$ sa classe
d'échec, $c$ un objet du noyau certifiant cette classe, $\mathit{rebroussement}$ le
...
\bottomrule
\end{tabular}
\caption{La taxonomie des échecs. Chaque classe est instanciée par un cas réel de la
semaine instrumentée, vérifié par le noyau.}
\end{table}
...
Toute notion formalisée porte une ancre lisible par machine
\texttt{@livre Ch.X \S{}s.ss Type.n | E X.pp L.a-b | PDF p.n} la reliant à la source
imprimée à la granularité page-et-ligne (2\,181 notions ancrées à l'heure où nous
écrivons). Soit $\Phi$ la transcription des énoncés numérotés du livre. La fidélité
...
\paragraph{Reproductibilité.} Toutes les mesures ont été prises sur une seule machine
Windows~11~: Python~3.13, pytest~9.0.3 avec \texttt{pytest-xdist} à quatre workers,
...
Deux jours après la semaine rapportée ci-dessus (5--6 août), la même
instrumentation a été tournée vers un énoncé \emph{ouvert}~: la conjecture de
Goldbach, écrite sur l'arithmétique construite du corpus. L'épisode teste ce que
les sections précédentes promettent --- que les défauts d'énoncé se
\emph{démontrent}, que la dette se \emph{mesure}, et qu'un problème ouvert se
\emph{transforme} en théorèmes clos.

\emph{(i) Deux défauts d'énoncé, démontrés dans le noyau.} La première rédaction
de la conjecture disait «~$n$ pair et $n \neq 2$~». Son antécédent est satisfait
en $n = 0$ --- démontré, clos~: $\vdash \mathrm{pair}(0)$ (témoin $k := 0$) et
$\vdash \neg(0 = 2)$ --- de sorte que l'énoncé affirmait que \emph{zéro est somme
de deux nombres premiers}. Réparé, il restait faux~: «~pair~» contraint $n$ à
être un \emph{cardinal}, nullement un entier, et l'antécédent est aussi satisfait
par $n := \mathbb{N} + \mathbb{N}$, qui est infini --- démontré également, l'égalité
avec l'ancien antécédent instancié étant vérifiée formule contre formule. La même
faute --- un quantificateur posé sur les ensembles quand on le croit posé sur les
entiers --- avait été commise la veille sur le $(\forall d)$ de la primalité~:
c'est un \emph{motif}, désormais consigné. Fait notable~: la variante
\emph{démontrée} (bornée) de l'énoncé portait la garde de finitude depuis son
premier jet, parce que sa preuve l'exigeait~; seul l'énoncé \emph{non démontré}
avait dérivé. Démontrer force à nommer les hypothèses~; énoncer ne force rien.

\emph{(ii) Le théorème d'équivalence.} Le résultat principal de l'épisode est
clos, sans borne, sans analyse de cas sur $n$~:
\[
\vdash\;\bigl(H \Rightarrow \mathrm{Goldbach}\bigr)\;\wedge\;
\bigl(\mathrm{Goldbach} \Rightarrow H\bigr),
\qquad
H \;=\; (\forall k)\bigl(\,\mathrm{Fini}\,k \wedge k \neq 0 \wedge k \neq 1
\;\Rightarrow\; (\exists p)(\exists p')\,(p, p' \text{ premiers} \wedge k+k = p+p')\,\bigr).
\]
La conjecture \emph{est} sa forme «~moitiés~»~: les deux énoncés sont
interdérivables, et ce qui manque au problème ouvert est isolé en un point
unique --- produire la décomposition. La fidélité des deux formules est établie
par \emph{prélèvement}~: l'antécédent et le conséquent sont découpés dans
l'énoncé du dépôt puis la recomposition est vérifiée à l'identique, de sorte
qu'aucune recopie ne peut mentir. La direction réciproque traverse un
$\alpha$-renommage forcé (le témoin de parité porte le même nom que la variable
instanciée)~; le lieur frais est \emph{lu sur le théorème instancié} plutôt que
prédit. L'aller ne contient aucune analyse de cas~; la réciproque en contient
exactement une, déclarée~: $k \in \{0,1,2\}$, taille fixe, sur la constante $2$
de l'énoncé.

\emph{(iii) La dette, en trois nombres.} «~Zéro hypothèse résiduelle~» et
«~l'invariant des 22 axiomes tient~» se lisent ensemble comme «~ne dépend que des
22~» --- c'est faux au sens mesuré, puisque les axiomes entrent par la règle
$\mathrm{axiome}$ et disparaissent du séquent (section~\ref{sec:certified-failure}).
Mesuré en process frais sur la forme bornée en $n$~: \textbf{0} hypothèse
résiduelle~; \textbf{14} des 22 axiomes de référence consommés~; \textbf{53}
formules étrangères --- une seule règle, le critère C54 (le graphe d'une fonction
définie par un terme, théorème chez Bourbaki, axiome de théorie dédiée ici),
instanciée 52 fois, plus une instance de Knaster--Tarski. Un résultat honnête
s'annonce avec les trois nombres~; le troisième dit ce qui reste à dériver. Sur
l'équivalence elle-même~: \textbf{0}~; \textbf{14}~; \textbf{73} --- réparties
sur \emph{21} théories dédiées, la machinerie du bon ordre (Zorn, Zermelo,
Bourbaki--Witt) entrant par le lemme «~un minorant d'un fini est fini~».
Enlever la borne se paie en bon ordre~: le passage de 2 à 21 théories
\emph{mesure} ce que la généralité coûte. La première mesure avait rendu zéro en
une seconde --- le théorème sortait d'un cache, et l'instrument est aveugle à ce
qui ne se construit pas~: la règle du process frais, écrite pour d'autres
mesures, s'applique à elle-même.

\emph{(iv) La machine invente.} Le troisième jour, les organes de découverte des
sections~\ref{sec:certified-failure}--\ref{sec:vectors} ont été braqués sur
l'îlot. Le conjectureur (transitivité + détachement, noyau seul juge) a certifié
60~théorèmes en trois tours, retrouvant seul les lemmes de liaison que la
campagne avait écrits à la main~; quatre ont été \emph{promus} au dépôt avec leur
provenance --- les premiers théorèmes du corpus dont l'origine est la machine.
Le régime existentiel a d'abord péri sur sa propre représentation ---
$2+2=4$ \emph{déplié} dépasse $400\,000$ nœuds là où son DAG partagé en compte
une cinquantaine --- puis, réécrit contre la structure partagée, a livré
l'observation décisive~: dans ce corpus le niveau arithmétique est $\tau$, les
constructeurs d'application ne sont que la plomberie de l'encodage. Enfin,
l'abstraction \emph{sélective} --- n'abstraire que dans \emph{un membre} d'une
égalité, ce qui satisfait S5 malgré l'emboîtement des numéraux, en coût
linéaire --- a permis à la machine d'\emph{inventer des notions}~: de
$\vdash 6 = 3+3$ elle produit exactement $\mathrm{pair}(6)$ --- la formule du
module d'énoncé, comparée caractère à caractère --- et de même $\mathrm{pair}(4)$
et $\mathrm{pair}(10)$, plus 57~existentiels de complément et de cofacteur,
chacun certifié par le noyau, en une seconde par passe.

Le cycle s'est refermé quand la machine a \emph{consommé} sa propre invention~:
$\mathrm{pair}(6)$, assemblée aux quatre conjoints restants du dépôt par un organe
de détachement conjonctif --- dont la leçon est que, les définitions étant
elles-mêmes des conjonctions, l'aplatissement doit s'arrêter \emph{aux faits
connus} --- puis détachée à travers le théorème borné, redonne la décomposition
de $6$, close, sans hypothèse. En série, $6$, $8$ et $10$ tombent en un dixième
de seconde chacune après une chauffe unique~; et chaque site d'introduction
existentielle de la campagne passe désormais par la tactique à témoin vérifié
que cette moisson a fait émerger.

\emph{(v) La machine compose.} Le quatrième jour a fermé la boucle. Le volant
wake-sleep --- abstraction des motifs partagés entre preuves, gate noyau,
critère de compression --- a été étendu aux prouveurs \emph{paramétrés} (36 des
43 preuves de l'îlot le sont~; le gate n'exécutait que du zéro-argument), avec
un voile sur les caches déclarés~: sans lui, un prouveur mémoïsé rend son
théorème en cache et le gate ne teste rien. Chaque tour est devenu
\emph{observable} --- un flux d'événements nomme la source en cours, chaque
découverte certifiée, chaque brique écartée --- et ce flux a payé aussitôt~:
deux gels que la version muette avait facturés une nuit entière ont été
localisés en minutes (la déduplication et le tri des briques marchaient sur les
arbres \emph{dépliés}~; réécrits contre la structure partagée, cinq tours de
découverte passent de trois heures gelées à dix minutes). Le cycle complet
tourne alors sans à-coup~: 64~théorèmes découverts en flux, trois promus comme
lemmes de \emph{profondeur~2} --- le premier du dépôt dérivé à travers un lemme
machine --- et le tour de compression suivant les absorbe~: la bibliothèque
passe de 4 à 18~notions certifiées (gain de compression 80~pas, 42~preuves du
dépôt raccourcies, 96~macros d'ordre~2 consommant les notions). La troisième
instance d'un motif que la machine tenait à gain nul, apportée par son propre
lemme, l'a fait passer au positif~: l'invention de la veille finance la
compression du lendemain.

L'épisode a coûté quatre jours, ajouté sept modules et une trentaine de tests au
dépôt (suite finale~: 152 verts, équivalence redémontrée en process frais), et
laissé le compte d'axiomes à 22. Aucun des théorèmes ci-dessus n'approche la
conjecture elle-même~; tous bornent exactement ce qui lui manque --- et la
machine parle désormais, théorème par théorème, le langage dans lequel ce
manque s'énonce.
...
suite complète tourne en 2\,h\,43 à quatre workers et l'environnement tue les longues
...
\node[ax, font=\tiny] at ($(T1.center)+(-9mm,-5mm)$) (a22) {22 axiomes};
\node[draw=orange!85!black, fill=orange!14, rounded corners=2pt, font=\tiny] at ($(T1.center)+(9mm,-5mm)$) (delta) {$\delta$~: pont fam$\leftrightarrow$valeur};
...
corpus existant de 4\,099 tests en $10^5$--$10^6$ tuples certifiés (état, action,
