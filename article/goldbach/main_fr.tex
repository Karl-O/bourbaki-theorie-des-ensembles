...
un objet du noyau ou une mesure énoncée, jamais une estimation. Et parce que
$\exists x\,\varphi(x)$ \emph{est} $\varphi(\tau x\,\varphi)$ chez Bourbaki plutôt qu'un
quantificateur séparé, la conjecture peut être réécrite sans aucun quantificateur
existentiel --- trois propriétés de deux termes nommés (section~\ref{sec:map}). Cela n'est
pas disponible dans un cadre conventionnel, et il se trouve que c'est la forme sous
laquelle les organes de recherche de la machine re-dérivent plus tard l'énoncé d'eux-mêmes.
...
Pour une relation $R$ et une lettre $x$,
\[ (\exists x)R \;:=\; (\tau_x(R) \mid x)\,R , \]
où $(A \mid x)\,R$ désigne l'assemblage obtenu en substituant $A$ à toute occurrence de
$x$ dans $R$. Le terme $\tau_x(R)$ dénote « un objet satisfaisant $R$, s'il en existe »~;
l'énoncé existentiel \emph{est} donc l'énoncé que ce terme nommé-là satisfait $R$.
...
\paragraph{La conjecture sans quantificateur existentiel.} En appliquant deux fois
$(\exists x)\varphi \equiv \varphi(\tau x\,\varphi)$, avec
$T := \tau p\,(\exists q\;\mathrm{mat})$ et $Q := \tau q\,(\mathrm{mat}[p := T])$, la
conjecture devient
\[
...
exact d'eux-mêmes~: un organe \emph{proposeur} de témoins qui fabrique $\tau x(\varphi)$
...
Le lemme de soutien est de l'arithmétique cardinale pure, sans aucun nombre premier~: si
$2k = m + m'$ avec les trois finis, alors $m \le k$ ou $m' \le k$. Sa preuve évite
entièrement les inégalités strictes --- elles coûtent cher dans ce noyau --- en utilisant
la comparabilité pour ouvrir deux cas, l'existence d'un complément dans le cas $k \le m$
pour écrire $m = k + d$, puis la simplification additive finie sur
$k + k = (k+d) + m' = k + (d + m')$ pour obtenir $k = d + m'$.
...
\paragraph{Le comptage brut.} Le critère des tiroirs $2 \cdot \pi(2k) > 2k+1$ --- qui
forcerait $P_{2k}$ et son miroir à se rencontrer par la seule cardinalité --- ne tient
pour aucun $k \ge 2$. La densité $\pi/k$ tombe de $0{,}50$ à $0{,}18$ à
$2k = 2 \cdot 10^5$, alors que le nombre réel de décompositions \emph{croît} (de 2 à
1417) sur la même plage. C'est une mesure numérique, pas une preuve. Sa conséquence a été
une décision~: ne pas formaliser l'inclusion--exclusion pour ce problème. Ce qu'il faut,
c'est de l'information sur la \emph{répartition} de $P_{2k}$, pas sur son cardinal.
