...
\texttt{preprint-v1} (\texttt{54ceef5}).
Toutes les mesures de cet article ont été refaites en processus frais les 19 et
20~août 2026~; le script de rejeu est \texttt{recherche/goldbach/capstone.py}.}}
...
\noindent
\textbf{Notation.} Nous écrivons $\vdash R$ lorsque le noyau peut construire un théorème
clos de conclusion $R$, et $\Gamma \vdash R$ lorsqu'il en construit un dont les
hypothèses sont contenues dans $\Gamma$. Tous les $\vdash$ de cet article sont donc
\emph{témoignés par un objet}, jamais affirmés~: chacun correspond à une exécution que le
paquet rejoue (section~\ref{sec:repro}). C'est une notation métalinguistique sur le
noyau, non un signe d'une théorie qu'il manipule.
...
le signe $\square$} --- l'un des quatre signes logiques du livre, qui marque une position
occupée jadis par une variable liée. Le résultat \textbf{ne contient plus la lettre $x$}.
Bourbaki en donne l'exemple à la ligne suivante~:
\[
\tau_x(\,\in\, x\, y\,)
