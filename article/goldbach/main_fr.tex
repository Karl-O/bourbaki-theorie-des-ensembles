...
Former $\tau_x(A)$, c'est~: préfixer $A$ par le signe $\tau$, \textbf{lier} au $\tau$
chaque occurrence de la lettre $x$, puis \textbf{remplacer chacune de ces occurrences par
le signe $\square$}. Le résultat \textbf{ne contient plus la lettre $x$}. Bourbaki en
donne l'exemple à la ligne suivante~:
\[
\tau_x(\,\in\, x\, y\,)
\quad\text{\emph{est l'assemblage}}\quad
...
\end{tikzpicture}}}
\]
Le trait est \emph{le lien} lui-même~: il fait partie de l'assemblage, au même titre que
les signes. C'est lui qui dit quel $\tau$ gouverne quel $\square$, et c'est pourquoi un
assemblage est un \textbf{graphe} et non une chaîne de caractères --- deux $\square$
côte à côte peuvent être liés à des $\tau$ différents.

La notation « $\tau_x$ » est, elle, métalinguistique~: elle nous rappelle quelle lettre a
été abstraite, mais l'assemblage n'en porte aucune trace.
\end{definition}
...
\begin{remarque}[le raccourci tentant, et pourquoi il est faux]
Une première rédaction de cet article écrivait
$(\exists x)\varphi(x) = \varphi(\tau x\,\varphi)$. Tout y est fautif~: $\varphi$ n'est
pas une fonction qu'on applique~; « $\tau x\,\varphi$ » laisse croire que $x$ figure
encore dans le terme, alors que la définition~\ref{def:tau} vient de l'en effacer~; et le
signe $=$ n'est pas disponible à ce niveau (remarque~\ref{rem:signes}). La forme correcte
est celle de substitution ci-dessus.
\end{remarque}

\begin{remarque}[écrire un signe, c'est se placer dans la théorie qui l'a]\label{rem:signes}
Discipline que nous tenons dans tout l'article, et qui vaut d'être énoncée parce qu'elle
est facile à enfreindre sans s'en apercevoir. \textbf{Le signe $=$ n'appartient pas au
niveau où vit la définition~\ref{def:tau}}~: c'est un signe relationnel de poids~2 introduit en
E~I.38, avec les schémas qui le gouvernent, et une théorie qui ne les possède pas est
dite \emph{non égalitaire}. Écrire $=$ entre deux assemblages du \S I.1 reviendrait donc
à emprunter un signe à une théorie où l'on ne se trouve pas encore.

C'est pourquoi la définition~\ref{def:tau} dit « \emph{est l'assemblage} » et non
« $=$ »~: il s'agit d'une identité \textbf{métalinguistique} --- deux notations désignent
la même suite de signes et de liens ---, et non de la relation d'égalité d'une théorie
égalitaire. La règle générale~: \emph{utiliser un signe, c'est déclarer qu'on travaille
dans une théorie qui le possède.}

Les $=$ des sections~\ref{sec:structure} et suivantes sont, eux, parfaitement légitimes~:
l'arithmétique cardinale se développe dans la théorie des ensembles, qui \emph{est}
égalitaire. Le noyau, lui, ne peut pas arbitrer ce genre de faute --- il vérifie des
dérivations, pas le sens qu'on prête aux symboles dans une phrase de prose. C'est
exactement la classe de défaut dont traite la section~\ref{sec:fidelity}.
\end{remarque}
