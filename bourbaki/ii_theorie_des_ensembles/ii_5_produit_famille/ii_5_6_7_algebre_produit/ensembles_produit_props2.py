...
§5.6 — Cor. 2 de la Prop. 9 (distributivité × / ∪), sens « ⊂ » via membership :
  • produit_distrib_inter_membre
        ⊢ ( G∈∏_{ι∈I}(⋂_{κ∈K}X_{ι,κ})  et  κ₀∈K
              et (∀ι)(ι∈I ⇒ W_ι = ⋂_κ X_{ι,κ})
              et (∀ι)(ι∈I ⇒ V_ι = X_{ι,κ₀})
              et (∀ι)(ι∈I ⇒ (G(ι)∈⋂_κ X_{ι,κ} ⇒ G(ι)∈X_{ι,κ₀})) )
          ⇒ G∈∏_{ι∈I}X_{·,κ₀}                                          [CONDITIONNEL]
        — moitié de la commutation produit/intersection (Prop. 10, E.II.5.6) :
          ...
          des X_{·,κ}.  CINQ prémisses (l'énoncé est une implication à antécédent
          conjonctif de 5 membres, cf. le code) : la valeur (⋂_κ X_{ι,κ}) au
          facteur ι est bien l'intersection ⋂_κ (valeur) — déf. de la famille
          ι↦⋂_κ X_{ι,κ} —, idem pour le facteur but, ET l'inclusion
          ⋂_κ X_{ι,κ} ⊂ X_{ι,κ₀} au point G(ι), prise en prémisse elle aussi
          (les facteurs sont ici des TERMES ABSTRAITS, donc AXIOME_INTER_FAM ne
          s'y applique pas).  Le conjoint κ₀∈K figure dans l'antécédent mais
          n'est PAS consommé par la preuve (cf. audit 26 juil. 2026) : c'est un
          affaiblissement gratuit ANTÉRIEUR à la migration ⋂-par-sélection, à
          nettoyer — la version propre tire cette 5ᵉ prémisse de
          `inter_famille_incluse_facteur` sous κ₀∈K, sur de vrais ⋂_fam.

══════════════════════════════════════════════════════════════════════════════
MIGRATION ⋂-PAR-SÉLECTION (26 juil. 2026) — statut de CE fichier : TOUT en A.
══════════════════════════════════════════════════════════════════════════════
AXIOME_INTER_FAM a été remplacé (theorie_ensembles() reste à 22) : son membre
droit n'est plus (∀i)(i∈I ⇒ z∈X_i) mais la CONJONCTION
    ( z ∈ ⋃_{ι∈I}X_ι  ET  (∀i)(i∈I ⇒ z∈X_i) )
— l'ancienne forme rendait ⋂_{ι∈∅}X_ι universel, donc la théorie contradictoire
(preuve : outils_ia/audit/preuve_incoherence_inter_vide.py).

Un seul résultat d'ici touchait l'axiome : `inter_famille_incluse_facteur`.  Il
n'en consomme que la direction d'ÉLIMINATION, donc sa réparation se réduit à une
PROJECTION DROITE de la conjonction — ÉNONCÉ INCHANGÉ, aucune hypothèse ajoutée
(le témoin d'indice qu'exige désormais l'introduction est ici déjà présent comme
antécédent κ₀∈K).  Aucun théorème du fichier n'est passé en B ni en C, et le test
`test_produit_props2.py` n'a PAS eu à être modifié — c'est lui qui atteste que
l'énoncé est resté le même.  `produit_distrib_inter_membre` prend l'inclusion
⋂_κ ⊂ X_{·,κ₀} en PRÉMISSE sur des termes abstraits : il n'a jamais dépendu de
l'axiome et n'est pas concerné.
"""
from __future__ import annotations

from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import (Terme, var, app, egal, et, impl, non, equiv,
...
from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import (instancie, instanciation,
...
def produit_monotone_facteurs(f="f", g="g", i="I", ff="F", iota="i"):
    ...
    _incl, _fonct, _dom, forall_f = composants_membre(hF, vf, vI, vF)
    ...
    # transport ∏f → ∏g : conjoint de tête RECONSTRUIT (⋃Y_ι ≠ ⋃X_ι), les autres conservés
    FinG = transporter_dans_produit(hF, forall_g, vF, vf, vg, vI, iota)   # F∈∏g
...
def facteurs_egaux_donne_inclus(f="f", g="g", i="I", ff="F", iota="i"):
    ...
    FinG = transporter_dans_produit(hF, forall_g, vF, vf, vg, vI, iota)
...
def _vars_libres_terme(t):
    ...
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import libres_t
...
# @livre Ch.R §4 Prop.- | E.R.21 item 11 (associativité du produit : partition (J_λ) de I) | PDF p.324
...
# AXIOME_INTER_FAM, désormais sous forme de SÉLECTION DANS LA RÉUNION (§II.4.1
# Déf. 2, route Grimm B5) :
#     z∈⋂_K X_κ  ⇔  ( z∈⋃_K X_κ  ET  (∀κ)(κ∈K⇒z∈X_κ) ) ,
# donc le membre droit est une CONJONCTION : on en prend la PROJECTION DROITE
# avant d'instancier κ=κ₀ et de décharger κ₀∈K.  L'énoncé, lui, est INCHANGÉ :
# c'est une direction d'ÉLIMINATION (⋂ ⇒ chaque facteur), la seule que la
# réparation de l'axiome laisse gratuite (cf. `inter_donne_membres`).
...
def inter_famille_incluse_facteur(h_fam="h", k="K", kappa="k0", z="z"):
    ...
    dès que κ₀∈K.  Par AXIOME_INTER_FAM (forme de SÉLECTION, §II.4.1 Déf. 2) :
    z∈⋂ ⇔ ( z∈⋃ ET (∀κ)(κ∈K⇒z∈X_κ) ) ; à z fixé on PROJETTE À DROITE cette
    conjonction, on instancie le ∀κ en κ₀ et on décharge κ₀∈K, d'où z∈X_{κ₀} ;
    généralisation sur z (liant « z », celui de l'axiome).
    X_κ = valeur_famille(h, κ).  Lemme INCONDITIONNEL réutilisable
    (Prop. 10 §5.6, Cor. de la Prop. 9).

    STATUT MIGRATION (26 juil. 2026) — issue A : ÉNONCÉ INCHANGÉ.  L'ancien
    AXIOME_INTER_FAM (sans la borne ⋃) rendait ⋂_{κ∈∅}X_κ universel et la
    théorie CONTRADICTOIRE ; sa réparation par sélection ne coûte ici qu'un pas
    de projection, car ce lemme ne consomme QUE la direction d'élimination —
    exactement le cas « migre sans rien changer » documenté par
    `ensembles_inter_selection_ii4.inter_donne_membres`.  Aucune hypothèse
    (∃κ)(κ∈K) n'est requise : le témoin κ₀∈K est déjà l'antécédent de l'énoncé."""
    ...
    # AXIOME_INTER_FAM instancié à (h, K, z) :
    #   z∈⋂ ⇔ ( z∈⋃ et (∀κ)(κ∈K⇒z∈X_κ) )      [forme de sélection]
    ...
    conj = N.modus_ponens(h_zin, equivalence_avant(eq)) # z∈⋃ et (∀κ)(κ∈K⇒z∈X_κ)
    forall_k = conjonction_elim_droite(conj)            # (∀κ)(κ∈K⇒z∈X_κ)
...
def produit_distrib_inter_membre(f_inter="W", g_kappa="V", i="I", k="K",
    ...
    d'où G(ι)∈X_{ι,κ₀} = (facteur but) par la 5ᵉ prémisse, donc G∈∏(g_kappa).
    Rien postulé : les identifications de facteurs ET l'inclusion
    ⋂_κ X_{ι,κ} ⊂ X_{ι,κ₀} au point G(ι) sont des hypothèses — nécessairement,
    car ⋂_κ X_{ι,κ} est ici un TERME ABSTRAIT (`_inter_facteur`) et non un
    `inter_famille`, donc AXIOME_INTER_FAM ne s'y applique pas.

    ⚠️ DETTE (relevée par l'audit du 26 juil. 2026, ANTÉRIEURE à la migration
    ⋂-par-sélection : le conjoint κ₀∈K figure dans l'antécédent et n'est utilisé
    NULLE PART dans la preuve (il n'est jamais extrait de `h`).  C'est donc une
    hypothèse morte, qui affaiblit gratuitement l'énoncé.  Ne pas la lire comme
    la justification de la 5ᵉ prémisse : celle-ci est prise telle quelle.  À
    nettoyer en réécrivant la fonction sur de vrais ⋂_fam, où la 5ᵉ prémisse se
    DÉDUIT de `inter_famille_incluse_facteur` sous κ₀∈K."""
    ...
    # ⋂_κ X_{ι,κ} ⊂ X_{ι,κ₀} (à i fixé) : PRÉMISSE.  Elle N'est PAS tirée de
    # AXIOME_INTER_FAM ici — InterVal est un terme abstrait, pas un inter_famille ;
    # sur de vrais ⋂_fam elle se déduirait de `inter_famille_incluse_facteur` sous κ₀∈K.
    ...
    _incl_W, _fonct, _dom, forall_W = composants_membre(h_Gin_t, vW, vI, vG)
    ...
    GinG = transporter_dans_produit(h_Gin_t, forall_V, vG, vW, vV, vI, iota)   # G∈∏V
