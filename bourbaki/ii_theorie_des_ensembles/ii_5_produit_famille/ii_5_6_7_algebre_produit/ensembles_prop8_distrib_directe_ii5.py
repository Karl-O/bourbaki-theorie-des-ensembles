...
L'antécédent (∃i)(i∈I) EST l'hypothèse « I = ∏_{λ∈L} J_λ ≠ ∅ » que la Proposition 8
écrit noir sur blanc ci-dessus ; il est porté par l'énoncé depuis la migration
« ⋂ = sélection dans ⋃ » (2026-07-26).  La version antérieure de ce module concluait
l'inclusion SANS antécédent : elle n'était démontrable que via l'ancien
AXIOME_INTER_FAM, CONTRADICTOIRE (⋂ sur un ensemble d'indices vide y contenait tout
objet).  Avec l'axiome réparé ⋂_{f∈∅}(…) = ∅ et l'inclusion nue est FAUSSE — p. ex.
L = {a,b}, J_a = ∅, J_b = {1}, X_{b,1} ≠ ∅ : alors I = ∅ donc le membre droit vaut ∅,
tandis que le membre gauche contient X_{b,1}.  Le CONSÉQUENT, lui, est resté verbatim
l'ancienne cible (cf. `test_hypothese_I_non_vide_est_bien_l_antecedent`).
NB : les deux autres hypothèses de Bourbaki (L ≠ ∅, J_λ ≠ ∅ pour tout λ) ne sont PAS
requises par ce sens de l'inclusion et n'ont donc pas été ajoutées.
...
STRATÉGIE (preuve ponctuelle, calquée sur le plan validé) :
  hI = assume((∃i)(i∈I)) ; soit x ; h = assume(x ∈ ⋃_λ ⋂_ι X_{λ,ι}).
  (1) membre_reunion_famille (sens AVANT) : (∃λ)(λ∈L et x∈⋂_{ι∈J_λ}X_{λ,ι}).
  (2) existe_elimination : témoin λ₀, avec λ₀∈L et x∈⋂_{ι∈J_λ₀}X_{λ₀,ι}.
  (3) but x∈⋂_{f∈I}(…) : sens ARRIÈRE de `caracterisation_inter_famille_non_vide`
      (sous hI) ⇐ (∀f)(f∈I ⇒ x∈⋃_λ X_{λ,f(λ)}) ; f frais, hf = assume(f∈I).
  ...
  (7) loi_deduction(f∈I,…), generalisation(f), puis equivalence_arriere de l'équivalence
      restituée sous hI par `caracterisation_inter_famille_non_vide` → x∈⋂_f.
  (8) décharge témoin (existe_elimination), loi_deduction(x∈⋃…, but), generalisation(x),
      puis loi_deduction((∃i)(i∈I), …) qui décharge hI en ANTÉCÉDENT (x n'y est pas libre).

NOTE — le TÉMOIN d'indice ne pouvait PAS être obtenu gratuitement.  L'hypothèse
x∈⋂_{ι∈J_λ₀}X_{λ₀,ι} livre bien un témoin, mais dans J_λ₀ ; l'intersection que l'on
INTRODUIT est indexée par I = ∏_{λ∈L}J_λ, et passer d'un indice de J_λ₀ à un élément
de I demande une fonction de choix — précisément le cor. 2 de la prop. 5, hors cible.
D'où l'antécédent (∃i)(i∈I), qui est aussi ce qu'écrit Bourbaki.
...
# ── cible ─────────────────────────────────────────────────────────────────────
def _hyp_I_non_vide(j="J", L="L"):
    """(∃i)(i ∈ I),  I = ∏_{λ∈L} J_λ — l'hypothèse « I ≠ ∅ » de la Proposition 8."""
    return indices_non_vides(E.produit_famille(_t(j), _t(L)))


def _cible(xx="XX", j="J", L="L", gl="GL", gr="GR"):
    """⊢-cible : (∃i)(i∈I) ⇒ ⋃_{λ∈L}(⋂_{ι∈J_λ}X_{λ,ι}) ⊂ ⋂_{f∈I}(⋃_{λ∈L}X_{λ,f(λ)}).

    RENFORCEMENT D'ÉNONCÉ (2026-07-26).  L'antécédent « I ≠ ∅ » est celui que la
    Proposition 8 écrit noir sur blanc (E II.35 : « Soit I = ∏_{λ∈L} J_λ ≠ ∅ ») ;
    il était absent de la formalisation précédente, qui reposait sur l'ancien
    AXIOME_INTER_FAM — CONTRADICTOIRE, car il peuplait ⋂_{f∈∅} de tout objet.
    Sans cet antécédent l'inclusion est FAUSSE : pour I = ∅ le membre droit vaut ∅
    (⋂ = sélection dans ⋃) alors que le membre gauche n'a aucune raison d'être vide."""
    return impl(_hyp_I_non_vide(j, L),
                inclus(_membre_gauche(xx, j, L, gl), _membre_droit(gr, j, L)))
...
def distributivite_reunion_inter_inclusion_directe(
    ...
    ÉNONCÉ RENFORCÉ (migration « ⋂ = sélection dans ⋃ », 2026-07-26).  L'antécédent
    (∃i)(i∈I) — « I ≠ ∅ », hypothèse EXPLICITE de la Proposition 8 — est désormais
    porté par l'énoncé.  L'ancienne forme, SANS hypothèse, était FAUSSE pour I = ∅ :
    elle n'était démontrable que via l'ancien AXIOME_INTER_FAM, contradictoire (⋂ sur
    un ensemble d'indices vide y contenait tout objet).  La conclusion x ∈ ⋂_{f∈I}(…)
    exige maintenant AUSSI x ∈ ⋃_{f∈I}(…), donc un TÉMOIN d'indice dans I ; ce témoin
    est le τ-terme livré par `caracterisation_inter_famille_non_vide`, qui restitue
    sous cette hypothèse l'équivalence de l'ancien axiome.  Le reste de la preuve
    (élimination sur ⋂_{ι∈J_λ₀}, introduction dans ⋃_{λ∈L}) est inchangé."""
    vxx, vj, vL = _t(xx), _t(j), _t(L)
    vgl, vgrin, vgr, vff = _t(gl), _t(grin), _t(gr), _t(ff)
    vx = var("z")                       # élément courant (= liant de inclus)
    vI = E.produit_famille(vj, vL)      # I = ∏_{λ∈L} J_λ
    hyp_I = indices_non_vides(vI)       # (∃i)(i ∈ I) — « I ≠ ∅ » (E II.35)
    hI = N.assume(hyp_I)
    ...
    # INTRODUCTION dans ⋂GR : elle exige un témoin d'indice dans I (motif (b) de la
    # migration).  `caracterisation_inter_famille_non_vide` le fabrique (τi(i∈I)) à
    # partir de hyp_I et restitue l'équivalence de l'ANCIEN axiome sous cette hypothèse.
    eq_all = N.modus_ponens(hI, caracterisation_inter_famille_non_vide(vgr, vI, "z"))
    eq_inter = instancie(eq_all, vx)                          # (x∈⋂GR) ⇔ (∀i)(i∈I ⇒ x∈GR(i))
    x_in_interGR = N.modus_ponens(forall_i, equivalence_arriere(eq_inter))  # x ∈ ⋂GR  {hLam,hI}
    ...
    # décharge de l'hypothèse « I ≠ ∅ » : z n'y est pas libre, la généralisation est licite
    res = N.loi_deduction(hyp_I, incl)                        # (∃i)(i∈I) ⇒ (⋃GL ⊂ ⋂GR)

    assert res.conclusion == _cible(xx, j, L, gl, gr), \
        "distributivite_reunion_inter_inclusion_directe : conclusion ≠ cible"
    assert res.hypotheses == frozenset(), \
        "distributivite_reunion_inter_inclusion_directe : doit être CLOS"
    return res
...
def _proj_et_gauche(h_et, formule_et):
    ...
    from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import conjonction_elim_gauche
    return conjonction_elim_gauche(h_et)


def _proj_et_droite(h_et, formule_et):
    """De Γ⊢(A et B) déduire Γ⊢B."""
    from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import conjonction_elim_droite
...
def _inter_elim_terme(fam, idx, a, z):
    ...
    ÉNONCÉ INCHANGÉ par la migration « ⋂ = sélection dans ⋃ » (issue A) : c'est la
    direction d'ÉLIMINATION, celle qui reste inconditionnelle (cf.
    `ensembles_inter_selection_ii4.inter_donne_membres`).  Seule la PREUVE change :
    le membre droit de l'axiome est désormais la CONJONCTION
    ( z∈⋃_{ι∈idx} fam_ι  et  (∀i)(i∈idx ⇒ z∈fam_i) ), dont on prend la projection
    DROITE avant d'instancier au témoin a."""
    ax = N.axiome(E.theorie_ensembles(), E.AXIOME_INTER_FAM)
    inst = instancie(instancie(instancie(ax, fam), idx), z)  # (z∈⋂)⇔(z∈⋃ et (∀i)(…))
    h = N.assume(appartient(z, E.inter_famille(fam, idx)))
    conj = N.modus_ponens(h, equivalence_avant(inst))        # z∈⋃ et (∀i)(i∈idx⇒z∈fam_i)
    forall = conjonction_elim_droite(conj)                   # (∀i)(i∈idx⇒z∈fam_i)
...
# `_membre_inter_terme` SUPPRIMÉ (migration du 2026-07-26) : l'instance directe de
# l'axiome ⋂ ne donne plus l'équivalence avec (∀i)(…) mais avec la CONJONCTION
# (z∈⋃ et (∀i)(…)).  Le sens ⇐ passe désormais par
# `caracterisation_inter_famille_non_vide`, sous l'hypothèse « I ≠ ∅ ».


__all__ = ["distributivite_reunion_inter_inclusion_directe", "_cible",
           "_hyp_I_non_vide", "theorie_distrib"]
