...
PRÉMISSES (non vacuous — la conclusion x=y n'y figure pas) :
...
⚠️ CE QUI A CHANGÉ LE 26 juil. 2026, ET CE QUI N'A PAS CHANGÉ.  Jusqu'à cette date,
`AXIOME_PRODUIT_FAM` n'exposait QUE est_fonctionnel, dom = I et les valeurs — PAS
« tout élément est un couple » — et les deux `est_un_graphe` étaient de véritables
HYPOTHÈSES HONNÊTES, faute de mieux.  L'axiome ayant retrouvé son conjoint de tête
« F ⊂ I × ⋃_{ι∈I} X_ι » (préambule de la Déf. 1), elles sont désormais DÉMONTRABLES :
`ensembles_produit_famille.produit_graphe` est CLOS.  Elles sont donc REDONDANTES —
et pourtant elles restent dans l'énoncé, DÉLIBÉRÉMENT : la conclusion de ce théorème
est consommée telle quelle par quatre modules du chapitre III (prop2_injectivite,
cone_unicite, ordre_produit_antisym, adjonction_injective) et par trois tests
d'égalité EXACTE.  Alléger de 5 à 3 prémisses est correct mathématiquement mais
change une conclusion publique : c'est un CHANTIER SÉPARÉ, à mener en une passe
avec ses consommateurs.  Un appelant qui n'a pas les deux `est_un_graphe` sous la
main les obtient maintenant gratuitement par `produit_graphe`.
...
# @livre Ch.R §4 Prop.- | E.R.21 item 11 (z = (pr_ι(z))_{ι∈I} : extensionnalité du produit) | PDF p.324
...
def _fonctionnel_imp(vf, vI, vF):
    ...
    Chemin g,g,d depuis la réparation du 26 juil. 2026 (le conjoint de tête
    « F ⊂ I×⋃X_ι » a fait descendre `est_fonctionnel` d'un cran).  ⚠️ SANS ce
    ré-adressage la fonction CONSTRUIT ENCORE un théorème clos — de conclusion
    silencieusement fausse (« F∈∏ ⇒ (F⊂I×⋃X_ι et fonct F) ») : seul un test
    d'égalité EXACTE de la conclusion le détecte."""
    h = N.assume(appartient(vF, E.produit_famille(vf, vI)))
    corps = N.modus_ponens(h, equivalence_avant(_inst_produit(vf, vI, vF)))
    fonctionnel = conjonction_elim_droite(
