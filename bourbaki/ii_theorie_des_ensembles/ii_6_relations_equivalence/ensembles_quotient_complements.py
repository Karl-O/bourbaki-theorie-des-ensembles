...
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import (Terme, var, app, egal, et, impl, equiv,
...
def est_systeme_representants(S, g, e):
    ...
    `est_bijective(p|S, S, E/R)`  (Déf. 10, E II.17)."""
...
# @livre Ch.R §5 Def.- | E.R.24 item 8 (f compatible avec R et S) | PDF p.327
...
def application_deduite_quotient(f, p, h):
    ...
    H∘P applique d'abord p puis h).  f, p, h sont les GRAPHES.
    ✅ L'EXISTENCE effective de h n'est plus reportée : voir
    `ii_6_5_decomposition/ensembles_c57_passage_quotient.py`
    (c57_application_deduite : H := graphe_terme(Q, f(τz(t=p(z))), t) construit
    par le kit C54 sur le témoin canonique, ⊢ H(p(x))=f(x) sous 3 hypothèses
    honnêtes, SANS axiome du choix).  L'UNICITÉ de h reste, elle, à écrire
    (elle suit de la surjectivité de p, motif coincidence_sur_quotient)."""
    return egal(_tv(f), E.composee(_tv(h), _tv(p)))


# @livre Ch.II §6.5 Def.- | E II.45 L.1-2 | PDF p.96
# @livre Ch.R §5 Def.- | E.R.25 item 8 (h : E/R → F/S déduite de f par passage aux quotients) | PDF p.328
...
# @livre Ch.R §5 Def.- | E.R.24 item 5 (relation induite R_A) | PDF p.327
...
# Fin du texte du chapitre II — page E II.48 (le texte s'achève L.12 ; les
# Exercices commencent E II.49) :
#  L.1-6  : conclusion de §6.9 — R{x,x} entraîne A{x} = f(θ{x}) pour f : t ↦ A{t} ;
#           cas particulier : R équivalence dans un ensemble F, A{x} = classe de x,
#           f bijection de Θ sur F/R.  Cette bijection Θ → F/R n'est PAS formalisée.
#  L.7-12 : Exemple étoilé — « x et y sont deux espaces vectoriels de même dimension
#           finie sur C », relation sans graphe vérifiant la condition (1) — prose,
#           rien à formaliser.
# @livre Ch.II §6.9 Rem.- | E II.48 L.1-6 | PDF p.99
# @livre Ch.II §6.9 Ex.- | E II.48 L.7-12 | PDF p.99
