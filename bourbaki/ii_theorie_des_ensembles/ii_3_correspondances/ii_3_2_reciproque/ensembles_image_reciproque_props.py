...
from bourbaki.ii_theorie_des_ensembles.ii_3_correspondances.ii_3_4_fonctions.ensembles_extensionnalite import couple_dans_dom
...
def cible_inclus_image_reciproque_image(f="f", x="X"):
    # ⚠️ réparée le 8 août 2026 (balayage GP10) : l'antécédent H_app manquait —
    # la compagne datait d'avant son internalisation dans la conclusion.
    ...
    return impl(_hyp_applicative(vf, vx),
                inclus(vx, E.image(E.reciproque(vf), E.image(vf, vx))))
...
def cible_image_image_reciproque_inclus(f="f", y="Y"):
    # ⚠️ réparée le 8 août 2026 (balayage GP10) : l'antécédent est_fonctionnel
    # manquait — même dérive que la compagne de (18).
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import inclus
    vf, vy = _t(f), _t(y)
    return impl(E.est_fonctionnel(vf),
                inclus(E.image(vf, E.image(E.reciproque(vf), vy)), vy))
...
def image_reciproque_image_egal_si_injective(f="f", x="X"):
    ...
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import egal as _egal  # noqa
    vf, vx = _t(f), _t(x)
    recipf = E.reciproque(vf)
