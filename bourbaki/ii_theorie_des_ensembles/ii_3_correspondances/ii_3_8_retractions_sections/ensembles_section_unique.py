...
def cible_section_unique_par_image(s="S", sp="Sp", f="F", b="B"):
    ...
    on reproduit ici la forme capture-saine avec le MÊME liant canonique « @0 »
    (aligné le 8 août 2026, balayage GP10) : la cible est ÉGALE (==) à la
    conclusion du noyau, et reste α-équivalente pour tout consommateur en
    alpha_egal — le choix du nom de variable liée n'a aucune portée sémantique."""
    vS, vSp, vF, vB, vy = _T(s), _T(sp), _T(f), _T(b), var("y")
    lhs = E.valeur(vF, E.valeur(vS, vy, b="@0"), b="@0")   # f(s(y))   (τ « @0 »)
    rhs = E.valeur(vF, E.valeur(vSp, vy, b="@0"), b="@0")  # f(s'(y))  (τ « @0 »)
