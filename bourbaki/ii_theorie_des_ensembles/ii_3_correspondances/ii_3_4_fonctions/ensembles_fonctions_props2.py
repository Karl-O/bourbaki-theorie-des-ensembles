...
Ce module fournit les « directes » RESTANTES, au niveau VOCABULAIRE §II.3.7 (Déf. 10, E II.16)
et CORRESPONDANCES (§II.3.3), chacune NON présente ailleurs :
  ...
  • THÉORÈME 1 a+b) — la composée de deux bijections est bijective AU SENS Déf. 10 (E II.16-17)
      (`composee_bijectives`) : ⊢_{f:X→Y, f':Y→Z applications}
      (est_bijective(F,X,Y) et est_bijective(G,Y,Z)) ⇒ est_bijective(G∘F,X,Z).
      `est_bijective` (E II.17, Déf. 10) = injective_dans ∧ surjective — DISTINCT du
...
from bourbaki.i_description_mathematique_formelle.i_4_theories_quantifiees.i_4_3_tactiques_abrege_quantif import (congruence_existe,
                               ...
                               existe_elimination)
from bourbaki.ii_theorie_des_ensembles.ii_3_correspondances.ii_3_1_graphes_correspondances.ensembles_correspondances import _inst_image
from bourbaki.ii_theorie_des_ensembles.ii_2_couples.ii_2_1_definition_couples.ensembles_couples import membre_paire_gauche, singleton_membre
...
def composee_surjections(f="F", g="G", x="X", y="Y", z="Z"):
    ...
    surjection. »  Au niveau Déf. 10 (E II.16) : est_surjective(F,A,B) := F⟨A⟩=B.
...
# ── THÉORÈME 1 a+b) — la composée de deux bijections (au sens Déf. 10, E II.16-17) ──
# @livre Ch.II §3.8 Th.1 | E II.19 L.14-17 | PDF p.70
# @livre Ch.R §2 Prop.- | E.R.10 item 11 (la composée de deux bijections est bijective) | PDF p.313
def composee_bijectives(f="F", g="G", x="X", y="Y", z="Z"):
    """⊢_{F,G func, dom F=X, dom G=Y}
       (est_bijective(F,X,Y) et est_bijective(G,Y,Z)) ⇒ est_bijective(G∘F, X, Z).

    THÉORÈME 1 a+b combiné, au VOCABULAIRE Déf. 10 (E II.16-17) :
