...
Propositions formalisées VERBATIM (PDF E.II.25-27) :
  ...
  • Prop. 4 (E.II.25) — image réciproque d'une INTERSECTION de famille :
        ...
        {Fonctionnelle(f), α∈I} ⊢ f⁻¹⟨⋂_ι Y_ι⟩ = ⋂_ι f⁻¹⟨Y_ι⟩  (`image_recip_inter_egal`)
    — le sens ⊃ (et donc l'égalité) porte AUSSI l'antécédent α∈I, qui EST le
      « I ≠ ∅ » exigé par la Déf. 2 (E II.22) ; voir `image_recip_inter_arriere`.
  ...
  • Cor. de la Prop. 4 (E.II.25) — image directe d'une intersection sous INJECTION :
        {Injective(f), α∈I} ⊢ ⋂_ι Γ⟨X_ι⟩ ⊂ Γ⟨⋂_ι X_ι⟩  (`image_inter_arriere_si_inj`)
        {Injective(f), α∈I} ⊢ Γ⟨⋂_ι X_ι⟩ = ⋂_ι Γ⟨X_ι⟩   (`image_inter_egal_si_injective`)
    — là encore α∈I (le « I ≠ ∅ » de la Déf. 2) est un antécédent RÉEL des deux
      énoncés, pas seulement Injective(f).
...
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import (Terme, var, app, egal, et, non, impl,
...
# NOTE (migration Déf. 2, E II.22).  L'ancien `_inst_inter` — qui lisait l'axiome
# comme « z∈⋂ ⇔ (∀i)(i∈I ⇒ z∈X_i) » — A DISPARU : AXIOME_INTER_FAM est désormais la
# SÉLECTION DANS LA RÉUNION, dont le membre droit est une CONJONCTION.  Les deux
# gestes (élimination / introduction) passent par `ensembles_image_inter_pont_ii4`.
# L'ancienne lecture était FAUSSE pour I=∅ (elle peuplait ⋂_{ι∈∅} X_ι de tout objet).
...
def image_recip_inter_incluse(g="f", fam="Y", i="I"):
    ...
    ÉNONCÉ INCHANGÉ par la migration de la Déf. 2 (⋂ = sélection dans ⋃) : la
    conclusion a∈⋂f⁻¹⟨Y_ι⟩ exige désormais AUSSI un témoin d'indice, mais on le
    FABRIQUE — x∈⋂Y donne x∈⋃Y, donc un i₀∈I (`temoin_indice_via_inter`).  Aucune
    hypothèse « I≠∅ » n'est donc à ajouter : l'inclusion reste INCONDITIONNELLE."""
    ...
    fa = N.modus_ponens(x_inter, inter_elim(vfam, vI, vx))                      # (∀i)(i∈I⇒x∈Y_i)
    ...
    # témoin d'indice DÉRIVÉ de x∈⋂Y (⋂⊂⋃) : l'introduction dans ⋂ l'exige désormais
    T0, T0_in_I = temoin_indice_via_inter(vfam, vI, vx, x_inter)
    a_R = inter_intro(fam_r, vI, T0, T0_in_I, fa_i, va)                          # a∈⋂f⁻¹⟨Y·⟩
...
def image_recip_inter_arriere(g="f", fam="Y", i="I", a="alpha"):
    ...
    hL = N.assume(appartient(va, L))
    fa_L = N.modus_ponens(hL, inter_elim(fam_r, vI, va))  # (∀i)(i∈I⇒a∈(f⁻¹⟨Y·⟩)_i)
    ...
    # introduction dans ⋂ : le témoin d'indice est α, DÉJÀ en hypothèse (h_aI)
    x_inter = inter_intro(vfam, vI, valpha, h_aI, fa_x, vx)                        # x∈⋂Y
...
# @livre Ch.II §4.3 Prop.4 | E II.25 L.21-31 | PDF p.76
# @livre Ch.R §4 Prop.- | E.R.20 item 8 ((46) f⁻¹(⋂Y_ι)=⋂f⁻¹(Y_ι)) | PDF p.323
...
# @livre Ch.R §4 Prop.- | E.R.17 item 3 ((34) K(⋃X_ι)=⋃K(X_ι)) | PDF p.320
...
# @livre Ch.II §4.3 Prop.3 | E II.25 L.4-19 | PDF p.76
# @livre Ch.R §4 Prop.- | E.R.20 item 8 ((45) K(⋂X_ι)⊂⋂K(X_ι)) | PDF p.323
def image_inter_incluse(g="G", fam="X", i="I"):
    ...
    ÉNONCÉ INCHANGÉ par la migration de la Déf. 2 : le témoin d'indice qu'exige
    désormais l'introduction dans ⋂ est FABRIQUÉ à partir de x∈⋂X (⋂⊂⋃ donne un
    i₀∈I, `temoin_indice_via_inter`) — l'inclusion reste INCONDITIONNELLE."""
    vg, vfam, vI = _t(g), _t(fam), _t(i)
    ...
    gxy = cd(hb)
    hI = N.assume(appartient(vi, vI))
    fa = N.modus_ponens(x_inter, inter_elim(vfam, vI, vx))                      # (∀i)(i∈I⇒x∈X_i)
    ...
    # témoin d'indice DÉRIVÉ de x∈⋂X (⋂⊂⋃) : l'introduction dans ⋂ l'exige désormais
    T0, T0_in_I = temoin_indice_via_inter(vfam, vI, vx, x_inter)
    y_R = inter_intro(fam_im, vI, T0, T0_in_I, fa_i, vy)                          # y∈⋂Γ⟨X·⟩
...
def image_inter_arriere_si_inj(g="G", fam="X", i="I", a="alpha"):
    ...
    hL = N.assume(appartient(vy, L))
    fa_L = N.modus_ponens(hL, inter_elim(fam_im, vI, vy))  # (∀i)(i∈I⇒y∈(Γ⟨X·⟩)_i)
    ...
    # introduction dans ⋂ : le témoin d'indice est α, DÉJÀ en hypothèse (h_aI)
    x_inter = inter_intro(vfam, vI, valpha, h_aI, fa_x, vx)                         # x∈⋂X
