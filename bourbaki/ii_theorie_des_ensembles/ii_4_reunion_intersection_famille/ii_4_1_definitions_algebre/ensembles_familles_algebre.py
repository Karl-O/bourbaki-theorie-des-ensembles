...
prouvées comme énoncés autonomes :
  ...
  • BORNES de la famille (E.II.4.1, propriété fondamentale ⋂ ⊂ X_α ⊂ ⋃) :
    ...
    postulée — c'est l'hypothèse que Bourbaki écrit à la Déf. 2 (E II.22).

    NOTE MIGRATION (Déf. 2 réalisée par SÉLECTION dans ⋃, route Grimm).  Depuis que
    AXIOME_INTER_FAM a la forme  z∈⋂ ⇔ (z∈⋃ et (∀i)(i∈I ⇒ z∈X_i)),  les TROIS
    énoncés ci-dessus sont INCHANGÉS (issue A) : seules leurs preuves projettent
    désormais la conjonction.  Les deux premiers ont toujours besoin de α∈I (pour
    passer de (∀i)… à X_α, resp. pour le témoin d'indice de ⋃).  Le troisième,
    ⋂ ⊂ ⋃, est devenu INCONDITIONNEL — c'est littéralement la projection gauche de
    la sélection (`ensembles_inter_selection_ii4.inter_inclus_reunion`) ; on garde
    ici la forme conditionnée pour ne pas modifier l'énoncé publié.  L'ancienne
    justification « sans I≠∅, ⋂ serait l'univers et ⊄ ⋃=∅ » ne vaut PLUS : cette
    pathologie était la CONTRADICTION de theorie_ensembles(), et elle est morte
    (⋂_{ι∈∅} X_ι = ∅, cf. `ensembles_inter_migration_ii4`).
...
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import (Terme, var, egal, et, impl, appartient,
...
def _inst_inter(f, i, z):
    ...
    MIGRATION Déf. 2 (⋂ = SÉLECTION dans ⋃, route Grimm) : le membre droit est
    désormais une CONJONCTION.  L'ancienne forme (le seul membre (∀i)…) rendait
    theorie_ensembles() CONTRADICTOIRE — pour I=∅ elle peuplait ⋂ de TOUT objet
    (ensemble universel), cf. outils_ia/audit/preuve_incoherence_inter_vide.py.
    Les sites d'ÉLIMINATION se réparent en projetant la conjonction (`_corps_inter`)."""
    ax = N.axiome(E.theorie_ensembles(), E.AXIOME_INTER_FAM)
    return instancie(instancie(instancie(ax, f), i), z)


def _corps_inter(h_z_inter, f, i, z):
    """De ⊢ z∈⋂_{ι∈I} X_ι, tirer ⊢ (∀i)(i∈I ⇒ z∈X_i).   (Motif (a) — élimination.)

    Projection DROITE de la conjonction de sélection : c'est exactement le motif de
    `ensembles_inter_selection_ii4.inter_donne_membres`, mais joué ici sur l'axiome
    du corpus (theorie_ensembles(), 22 axiomes) plutôt que sur la théorie dédiée."""
    conj = N.modus_ponens(h_z_inter, equivalence_avant(_inst_inter(f, i, z)))
    return cd(conj)
...
# @livre Ch.R §4 Prop.- | E.R.19 item 8 (⋂X_ι ⊂ X_κ qq soit κ∈I) | PDF p.322
def inter_incluse_terme(f="X", i="I", a="a"):
    ...
    STATUT MIGRATION Déf. 2 (sélection) : ÉNONCÉ INCHANGÉ (issue A).  Seule la
    preuve bouge : le membre droit de l'axiome est devenu la conjonction
    (z∈⋃ et (∀i)…), on en PROJETTE la composante droite (`_corps_inter`) avant
    d'instancier en α.  L'hypothèse α∈I n'est plus nécessaire pour z∈⋃ (la
    sélection la fournit), elle reste requise pour passer de (∀i)… à z∈X_α."""
    ...
    fa = _corps_inter(hL, vf, vI, vz)                                   # (∀i)(i∈I ⇒ z∈X_i)
...
# @livre Ch.R §4 Prop.- | E.R.17 item 3 (X_κ ⊂ ⋃X_ι qq soit κ∈I) | PDF p.320
...
def inter_incluse_reunion(f="X", i="I", a="a"):
    ...
    Composition transitive de ⋂ ⊂ X_α (`inter_incluse_terme`) et X_α ⊂ ⋃
    (`terme_inclus_reunion`), toutes deux sous la même hypothèse α∈I.

    STATUT MIGRATION Déf. 2 (sélection) : ÉNONCÉ INCHANGÉ (issue A) ; la preuve
    hérite simplement de la réparation d'`inter_incluse_terme`.
    L'ancienne motivation de l'hypothèse (« sans α∈I l'inclusion est FAUSSE : ⋂_∅
    serait l'univers, ⋃_∅=∅ ») est CADUQUE : c'était précisément la pathologie de
    l'ancien AXIOME_INTER_FAM.  Avec ⋂ défini par sélection DANS ⋃, l'inclusion
    ⋂ ⊂ ⋃ est vraie INCONDITIONNELLEMENT (projection gauche de la sélection :
    `ensembles_inter_selection_ii4.inter_inclus_reunion`, CLOS, 0 hypothèse).
    On conserve ici la forme conditionnée par α∈I — plus faible mais fidèle à
    l'énoncé publié (E.II.4.1 encadre ⋂ ⊂ X_α ⊂ ⋃ sous « α ∈ I »)."""
    vf, vI, va = _t(f), _t(i), _t(a)
    vz = var("z")
    inter = E.inter_famille(vf, vI)
    reun = E.reunion_famille(vf, vI)
...
# @livre Ch.R §4 Prop.- | E.R.17 item 3 (J⊂I ⇒ ⋃_J X_ι ⊂ ⋃_I X_ι) | PDF p.320
