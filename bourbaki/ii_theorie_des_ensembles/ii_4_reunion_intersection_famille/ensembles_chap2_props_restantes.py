...
LOGIQUEMENT DIRECTES :
  ...
  • Proposition 1 (invariance par reparamétrage surjectif) — E.II.4.1 :
        si f : K → I et Y_κ := X_{f(κ)}, alors
            ⋃_{κ∈K} X_{f(κ)} ⊂ ⋃_{ι∈I} X_ι        (sous la SEULE hyp. de DOMAINE)
        et, SI f est surjective de K sur I, l'inclusion inverse, d'où l'ÉGALITÉ
            ⋃_{κ∈K} X_{f(κ)} = ⋃_{ι∈I} X_ι.
    Théorèmes : `reparam_reunion_incluse` (CONDITIONNÉ au domaine (∀κ)(κ∈K ⇒ φ(κ)∈I),
    sa conclusion est une IMPLICATION — jamais l'inclusion nue) et
...
from bourbaki.i_description_mathematique_formelle.i_5_theories_egalitaires.i_5_2_tactiques_abrege_egalite import symetrie as sym_eg
...
def _inter_equiv_si_temoin(h_a, a, f, i, z):
    """De ⊢ α∈I déduire ⊢ (z ∈ ⋂_{ι∈I} X_ι) ⇔ (∀i)(i∈I ⇒ z∈X_i).

    MIGRATION (Déf. 2, E II.22).  L'axiome AXIOME_INTER_FAM du corpus est désormais
    la forme de SÉLECTION dans ⋃ : son membre droit est une CONJONCTION
    ( z∈⋃ et (∀i)(i∈I ⇒ z∈X_i) ), et l'ancienne équivalence — FAUSSE pour I=∅, où
    elle peuplait ⋂ de TOUT objet — n'est valide que si I n'est pas vide.  Ici le
    témoin α∈I est déjà une hypothèse FIDÈLE de l'énoncé (Bourbaki : « pour tout
    α∈I »), donc (∃i)(i∈I) s'en déduit par S5 et le pont
    `caracterisation_inter_famille_non_vide` rend l'équivalence d'origine.
    Aucun renforcement d'énoncé n'est nécessaire."""
    vi = var("i")
    ex_i = N.modus_ponens(h_a, N.s5(appartient(vi, _t(i)), _t(a), "i"))  # (∃i)(i∈I)
    carac = N.modus_ponens(ex_i, caracterisation_inter_famille_non_vide(_t(f), _t(i)))
    return instancie(carac, _t(z))
...
def inter_constante(f="X", i="I", a="a"):
    ...
    MIGRATION Déf. 2 (sélection dans ⋃) — ÉNONCÉ INCHANGÉ.  La preuve consommait
    l'ancien AXIOME_INTER_FAM, dont le membre droit est maintenant la CONJONCTION
    ( z∈⋃X_ι et (∀ι)(ι∈I ⇒ z∈X_ι) ) ; les deux directions cassaient donc
    (élimination : « pas un (∀x)R » ; introduction : témoin d'indice manquant).
    Le théorème se re-prouve TEL QUEL parce que son hypothèse fidèle α∈I EST le
    témoin d'indice requis : `_inter_equiv_si_temoin` en tire (∃ι)(ι∈I) puis
    restitue l'équivalence d'origine via `caracterisation_inter_famille_non_vide`.
    Aucune hypothèse n'a été ajoutée à l'énoncé (le test est inchangé)."""
    vf, vI, va = _t(f), _t(i), _t(a)
    vz, vi = var("z"), var("i")
    Xi = E.valeur_famille(vf, vi)
    Xa = E.valeur_famille(vf, va)
    ...
    h_cst = cg(hH)
    h_a = cd(hH)
    # l'équivalence de la Déf. 2 récupérée grâce au témoin α∈I (cf. `_inter_equiv_si_temoin`)
    eq_inter = _inter_equiv_si_temoin(h_a, va, vf, vI, vz)

    # ── ⊂ : ⋂X_ι ⊂ X_α ──────────────────────────────────────────────────────
    hL = N.assume(appartient(vz, inter))
    fa = N.modus_ponens(hL, equivalence_avant(eq_inter))                   # (∀i)(i∈I ⇒ z∈X_i)
    ...
    z_inter = N.modus_ponens(forall_i, equivalence_arriere(eq_inter))
...
def reparam_reunion_incluse(f="X", phi="phi", i="I", k="K"):
    """⊢ (∀κ)(κ∈K ⇒ φ(κ)∈I) ⇒ ⋃_{κ∈K} X_{φ(κ)} ⊂ ⋃_{ι∈I} X_ι.  (E.II.4, Prop. 1 — sens facile.)

    CONDITIONNÉ à la seule hypothèse de DOMAINE (φ : K → I), fidèle au livre qui
    suppose φ application de K dans I.  ⚠ La conclusion RENDUE est l'IMPLICATION
    `impl(dom_hyp, inclus(⋃Y_κ, ⋃X_ι))`, PAS l'inclusion nue : ne pas comptabiliser
    ce résultat comme « inconditionnel » (le consommateur `lemme_8` de
    `outils_ia/decouvertes/lemmes_decouverts.py` décharge d'ailleurs bien dom_hyp).

    z∈⋃Y_κ ⇒ (∃κ)(κ∈K et z∈Y_κ) ; pour ce témoin κ, Y_κ = X_{φ(κ)}, donc z∈X_{φ(κ)}.
    Refermer par REUNION_FAM réclame le témoin d'indice ι := φ(κ) ACCOMPAGNÉ de
    φ(κ)∈I — l'axiome exige (∃i)(i∈I et z∈X_i), l'appartenance à I n'est pas
    gratuite —, d'où le chargement de dom_hyp = (∀κ)(κ∈K ⇒ φ(κ)∈I), hypothèse
    fidèle MINIMALE.
...
# @livre Ch.II §4.1 Prop.1 | E II.23 L.16-17 | PDF p.74
# @livre Ch.R §4 Prop.- | E.R.17 item 2 (la réunion ne dépend que de l'ensemble 𝔉) | PDF p.320
