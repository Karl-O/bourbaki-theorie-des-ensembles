...
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import (Terme, var, egal, et, non, equiv, appartient,
                     existe, subst_t)
...
from bourbaki.i_description_mathematique_formelle.i_5_theories_egalitaires.i_5_2_tactiques_abrege_egalite import (symetrie, symetrie as eg_symetrie,
                                      composer_egalites)
from bourbaki.i_description_mathematique_formelle.i_4_theories_quantifiees.i_4_3_tactiques_abrege_quantif import existe_elimination, congruence_existe, alpha_existe
...
from bourbaki.ii_theorie_des_ensembles.ii_2_couples.ii_2_1_definition_couples.ensembles_couples import singleton_injectif
from bourbaki.ii_theorie_des_ensembles.ii_5_produit_famille.ii_5_definitions.ensembles_produit_famille import membre_parties
...
def est_bijection_de_local(X, F, Y):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.definitions_cardinaux.ensembles_cardinaux import est_bijection_de as _b
    return _b(F, X, Y)
...
def cantor_non_equipotent(x="X"):
    ...
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import existe as _ex
    vX = _t(x)
    PX = E.parties(vX)
...
#   (« THÉORÈME 2 (Cantor). — Pour tout cardinal a, on a 2^a > a. » — le théorème
#    est dans la sous-section 6 « Relation d'ordre et opérations entre cardinaux »,
#    à la ligne 22 de la page ; ancienne citation §3.5 L.18-19 corrigée.)
# @livre Ch.III §3.6 Demo.2 | E III.30 L.23-28 | PDF p.133
#   (démonstration du livre : Card(𝔓(a)) = 2^a (Prop. 12), injection x↦{x},
#    argument diagonal X = {x∈a | x∉f(x)} — formalisée dans ce module entier.)
