...
def _valeur_eq_de_couple(vm, ve, vs):
    ...
    rebindé en « qi » (α-tau)."""
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.equipotence.ensembles_bijection import valeur_caracterisation
...
def _g_values_at_s(vg1, vg2, vs, vt, vm, s_nom):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_3_operations_cardinaux.iii_3_3_produit.ensembles_produit_equipotence import _antecedent_image
    vs_pt = var(s_nom)
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §3.5 Prop.9 | E III.28 L.29-32 | PDF p.131
#   (« Soient X et Y deux ensembles, a et b leurs cardinaux ; l'ensemble X^Y a pour
#    cardinal a^b. » — infra : l'invariance Eq(X,Y) ⇒ Eq(𝓕(X;A),𝓕(Y;A)) certifiée
#    ici assure la bien-définition de a^b sur les classes d'équipotence.)
