# LIVRE — couverture livre de `bourbaki/iii_ensembles_ordonnes_cardinaux_entiers/iii_3_equipotence_cardinaux`

> Généré par `python outils_ia/audit/gen_livre_manifestes.py` — NE PAS ÉDITER À LA MAIN.
> Page livre = pagination imprimée en haut de page (repère « E III.10 »), pas la page du PDF.

## Sous-dossiers (cumul)

| Dossier | Notions | À caler | Non conformes | Pages livre |
|---|---|---|---|---|
| `cantor/` | 2 | 0 | 0 | E III: 30 |
| `cantor_bernstein/` | 5 | 0 | 0 | E III: 25 |
| `definitions_cardinaux/` | 21 | 0 | 0 | E III: 5, 23-26, 28, 30 |
| `equipotence/` | 11 | 0 | 0 | E III: 23, 26 ; E R: 7, 13 |
| `iii_3_3_definition_ordre/` | 0 | 0 | 0 | — |
| `ordre_cardinaux/` | 18 | 0 | 0 | E III: 24-25, 30 |
| `parties_equipotentes/` | 6 | 0 | 0 | E R: 32 |
| `props_restantes/` | 15 | 0 | 0 | E III: 25-26, 28-30, 39 |
| `recollement/` | 5 | 0 | 0 | E III: 25, 28, 48 ; E R: 22 |
| `somme_produit_bornes/` | 7 | 0 | 0 | E III: 28, 30, 48 |

## Bilan cumulé (ce dossier + descendants)

- Notions calées : **90**
- Fichiers à caler : **0**
- Marqueurs non conformes : **0**
- Livre **E III** : pages couvertes 5, 23-26, 28-30, 39, 48 — **pages manquantes : 6-22, 27, 31-38, 40-47**
- Livre **E R** : pages couvertes 7, 13, 22, 32 — **pages manquantes : 8-12, 14-21, 23-31**

**Verdict : COUVERTURE INCOMPLÈTE — voir ci-dessus**
