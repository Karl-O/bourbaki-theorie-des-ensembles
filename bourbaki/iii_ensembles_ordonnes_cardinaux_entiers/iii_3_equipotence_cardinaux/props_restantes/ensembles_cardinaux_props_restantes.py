...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.somme_produit_bornes.ensembles_cardinaux_bornes_somme import inf_egal_somme_gauche
...
#   (« pour que l'on ait a ≥ b, il faut et il suffit qu'il existe un cardinal c
#    tel que a = b + c » — SENS RÉCIPROQUE (⇐), inconditionnel, certifié ici.)
...
#   (petit texte : « Si a ≥ b, il existe en général plusieurs cardinaux c tels que
#    a = b + c ; on ne peut donc en général définir une "différence" a − b … » —
#    prose sur la non-unicité du complément cardinal défini ici.)
...
# @livre Ch.III §3.6 Prop.13 | E III.29 L.50-54 | PDF p.132
#   (sens DIRECT (⇒), ici conditionné au complément cardinal ; version FERMÉE dans
#    props_restantes/ensembles_prop13_complement.py.)
