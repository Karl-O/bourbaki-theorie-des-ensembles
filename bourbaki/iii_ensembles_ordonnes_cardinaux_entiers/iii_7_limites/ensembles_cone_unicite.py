...
HYPOTHÈSES HONNÊTES — il n'en reste que TROIS (non vacuous — u=u' n'y figure pas) :
...
✅ La quatrième — (∀y)(y∈F ⇒ (graphe(u(y)) et graphe(u'(y)))), « les images sont
des ensembles de couples », prémisse de extensionnalite_produit — n'est PLUS
supposée.  La preuve établit déjà u(y), u'(y) ∈ lim← ; le lemme
`prop1_proj/ensembles_lim_graphe.point_limite_est_graphe` (§7.1) en tire le
caractère de graphe sans rien ajouter.  Le constructeur `cone_images_graphes`
reste exporté — il énonce cette prémisse historique — mais n'est plus consommé
ici.
...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_7_limites import ensembles_limites_iii7 as I7
from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import (
    instancie, conjonction_intro, conjonction_elim_gauche, conjonction_elim_droite,
    equivalence_avant)
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_3_operations_cardinaux.fondations.ensembles_graphe_de import graphe_de
...
def cone_coordonnees_egales(Efam, f, u, up, leq, i, ff, a="a", y="yy"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_7_limites.ensembles_limites_canoniques import application_canonique_proj_valeur
...
# @livre Ch.R §6 Prop.- | E.R.31 item 14 (g : E' -> E déduite des ga : unicité) | PDF p.334
def cone_unicite(Efam="E", f="f", u="u", up="up", leq=None, i="I", ff="F",
                 a="a", y="yy"):
    """{ u ∈ 𝓕(F;lim←), u' ∈ 𝓕(F;lim←),
         (∀α∀y)((α∈I et y∈F) ⇒ f_α(u(y)) = f_α(u'(y))) }  ⊢  u = u'.   [3 hyps].
    ...
    ✅ La quatrième prémisse historique — (∀y)(y∈F ⇒ (graphe(u(y)) et
    graphe(u'(y)))) — a DISPARU : la preuve établit déjà u(y), u'(y) ∈ lim←
    (image d'une application F→lim←), d'où leur caractère de graphe par
    `point_limite_est_graphe` (§7.1).  Elle n'était donc pas une hypothèse mais
    un fait, et l'énoncé s'en trouve strictement renforcé.
    ...
    # hypothèses (portées en Γ) — la prémisse « images graphes » n'en est plus une
    ...
    # graphe(u(y)), graphe(u'(y))   — DÉDUITS de u(y),u'(y) ∈ lim← (déjà établi
    # ci-dessus), via lim←⊂∏ puis « point du produit = graphe ».  Aucune
    # hypothèse ajoutée : c'est pourquoi `cone_images_graphes` n'est plus
    # consommée (elle reste exportée comme énoncé de la prémisse historique).
    ...
    g_u = point_limite_est_graphe(Efam, f, leq, i, u_y, u_in_lim)
    g_up = point_limite_est_graphe(Efam, f, leq, i, up_y, up_in_lim)
