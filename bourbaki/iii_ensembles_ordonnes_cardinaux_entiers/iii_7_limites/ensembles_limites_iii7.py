...
toucher aux fichiers déposés.  Il RÉUTILISE (import, AUCUNE modification) :
 - `bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_7_limites.ensembles_limites` (système projectif, lim_proj,
   axiome (1), appartient_limite_projective, limite_projective_relation_1) ;
 - `bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_7_limites.ensembles_limites_canoniques` (application canonique f_α,
...
✅ L'UNICITÉ de u (« et une seule ») N'EST PLUS REPORTÉE : elle est prouvée dans
`ensembles_cone_unicite.cone_unicite` (extensionnalité du produit +
application_egale_par_valeurs sur la limite), 4 hypothèses honnêtes.  Restent
REPORTÉS (champ REPORTES) : les critères 2° (u
...
# @livre Ch.III §7.2 Prop.1 | E III.53 L.5-6 | PDF p.156
# @livre Ch.R §6 Prop.- | E.R.31 item 14 (g : E' -> E déduite des ga : existence) | PDF p.334
...
REPORTES = [
    "Proposition 1, 1° UNICITÉ de u (« et une seule ») — ✅ FAIT depuis "
    "`ensembles_cone_unicite.cone_unicite` (4 hyps honnêtes : mêmes coordonnées, "
    "images dans lim←, images-graphes, u,u'∈𝓕(F;lim←)) ; ce report était PÉRIMÉ "
    "(corrigé le 4 août 2026).",
    "Proposition 1, 2° (u injective ⇔ (∀y≠z)(∃α) u_α(y)≠u_α(z)) — ✅ FAIT sous sa "
    "forme CONTRAPOSÉE dans `prop1_proj/ensembles_prop1_injectif."
    "coordonnees_egales_points` (mêmes coordonnées ⇒ même image, 6 hyps honnêtes) ; "
    "l'injectivité de u donne alors y=z.",
