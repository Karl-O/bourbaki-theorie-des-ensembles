...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_4_entiers_finis.iii_4_recurrence_c61_existence_n.ensembles_cardinal_pas_entre import bijection_implique_equipotent
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.definitions_cardinaux.ensembles_cardinaux_theoremes import cardinal_egal_si_equipotent
...
import bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_4_entiers_finis.ordinal_cardinal.realisation_segment.ensembles_realisation_segment_preuve as RSP
...
def bon_ordre_intervalle_depuis_subset(a="a", Ro="Ro", S="S"):
    ...
    ⚠⚠ ÉNONCÉ CHANGÉ PAR LA MIGRATION seg_ext (2026-07-31) — LIRE CECI.
    L'ancienne preuve éliminait `(∃Ro)` par ZERMELO en s'appuyant sur le fait que le
    terme de segment NE PORTAIT PAS l'ordre (`seg_ext(a,t)`) : la garde — et donc
    `subset_realise_segment` — passait pour « Ro-indépendante ».  C'ÉTAIT UN ARTEFACT
    DU DÉFAUT, le même qui rendait la théorie CONTRADICTOIRE (⊢ ∅∈∅ ; cf.
    docs/journal/ANOMALIES.md, 2026-07-31).  Le segment porte maintenant son graphe :
    `subset_realise_segment(Ro,a)` est Ro-DÉPENDANTE et le noyau REFUSE la
    généralisation (C27 : « 'Ro' libre dans une hypothèse » — MESURÉ).

    ⇒ L'HYPOTHÈSE EST DÉSORMAIS Ro-CLOSE : `subset_realise_segment_close(a)` =
        (∃Ro)( bo_form(Ro,a) ∧ subset_realise_segment(Ro,a) ),
    « il existe un bon ordre sur a dont TOUT sous-ensemble est équipotent à un segment
    initial ».  C'est l'IDIOME DÉJÀ EN PLACE dans le dépôt (`hyp_transport_ordinal`,
    E III.24).  L'élimination du ∃Ro redevient LÉGITIME.

    ⚠️ CONSÉQUENCE ASSUMÉE : ZERMELO SEUL NE SUFFIT PLUS (il donne un bon ordre, pas la
    réalisation des segments par CE bon ordre).  L'ancien énoncé était STRICTEMENT PLUS
    FORT et n'était démontrable que par le défaut.  NOTE : le capstone
    `bon_ordre_intervalle_close` (ensembles_gate_onto_top, CLOS, 0 hyp) est INTACT — il
    décharge sa garde AVANT de généraliser (test_gate_onto_top VERT, mesuré).
    """
    bo    = BOIO._bo_form_canon(a, Ro, _TPB, _BM, _BX)      # bo_form(Ro,a) bien formé
    srs   = subset_realise_segment(Ro, a)
    garde = realisation_segment_garde(Ro, a)
    boi   = bon_ordre_intervalle_sous_bo_form_garde(Ro, a, S)   # [bo_form, garde]
    assert set(boi.hypotheses) == {bo, garde}, "PIÈCE C gardée : hypothèses inattendues"

    # (1) la garde est DÉRIVÉE de subset_realise_segment (déjà le cas avant migration)
    rg = realisation_garde_depuis_subset(Ro, a)                 # [subset] ⊢ garde
    boi_sub = N.modus_ponens(rg, N.loi_deduction(garde, boi))   # boi  [bo_form, subset]
    assert set(boi_sub.hypotheses) == {bo, srs}, "hypothèses ≠ {bo_form, subset}"

    # (2) TOUT passer en ANTÉCÉDENT : `subset` AUSSI.  C'est LE point de la réparation :
    #     `subset_realise_segment(Ro,a)` porte Ro (le segment porte son graphe depuis la
    #     migration seg_ext), donc elle ne peut PAS rester hypothèse résiduelle pendant
    #     la généralisation sur Ro (C27).
    imp1 = N.loi_deduction(bo, boi_sub)                         # bo_form ⇒ boi   [subset]
    imp2 = N.loi_deduction(srs, imp1)                           # subset ⇒ (bo_form ⇒ boi)  CLOS
    assert imp2.est_clos, "imp2 devrait être CLOS"

    conj = et(bo, srs)
    h    = N.assume(conj)
    t1   = N.modus_ponens(conjonction_elim_droite(h), imp2)     # bo_form ⇒ boi   [conj]
    t2   = N.modus_ponens(conjonction_elim_gauche(h), t1)       # boi             [conj]
    imp_conj = N.loi_deduction(conj, t2)                        # conj ⇒ boi      CLOS
    assert imp_conj.est_clos, "imp_conj devrait être CLOS"

    # (3) le ∃Ro est MAINTENANT éliminable : plus AUCUNE hypothèse où Ro est libre.
    ex_imp = existe_elimination(imp_conj, Ro)                   # (∃Ro)conj ⇒ boi  CLOS
    H = subset_realise_segment_close(a, Ro)
    res = N.modus_ponens(N.assume(H), ex_imp)                   # boi   [ (∃Ro)conj ]

    assert res.conclusion == bon_ordre_intervalle(a),         "conclusion ≠ bon_ordre_intervalle(a)"
    assert set(res.hypotheses) == {H}, "hypothèse survivante ≠ la forme Ro-CLOSE"
    assert res.conclusion not in set(res.hypotheses), "VACUOUS"
    return res


def subset_realise_segment_close(a="a", Ro="Ro"):
    """L'HYPOTHÈSE, sous sa forme Ro-CLOSE (aucune variable Ro libre) :

        (∃Ro)( bo_form(Ro,a)  ∧  subset_realise_segment(Ro,a) )

    « il existe un bon ordre Ro sur a dont TOUT sous-ensemble est équipotent à un
    segment initial ».  MÊME IDIOME que `hyp_transport_ordinal` (E III.24) : le bon
    ordre et la propriété qu'on lui demande vivent sous LE MÊME ∃."""
    bo = BOIO._bo_form_canon(a, Ro, _TPB, _BM, _BX)
    return existe(Ro, et(bo, subset_realise_segment(Ro, a)))
...
def hypothese_unique(a="a", Ro="Ro"):
    """L'UNIQUE hypothèse SURVIVANTE ATTENDUE :
       { (∃Ro)( bo_form(Ro,a) ∧ subset_realise_segment(Ro,a) ) }.

    ⚠️ CHANGÉE par la migration seg_ext (2026-07-31) : c'était
    { subset_realise_segment(Ro,a) }, avec Ro LIBRE.  Cette forme-là ne suffit plus —
    cf. la docstring de `bon_ordre_intervalle_depuis_subset`."""
    return {subset_realise_segment_close(a, Ro)}
