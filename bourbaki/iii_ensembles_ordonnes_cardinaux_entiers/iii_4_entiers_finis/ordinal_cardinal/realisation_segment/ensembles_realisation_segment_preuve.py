...
from bourbaki.ii_theorie_des_ensembles.ii_1_relations_collectivisantes.ensembles_abrege import est_relation_ordre_dans
...
import bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.zorn_zermelo.ensembles_zermelo as Z
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.trichotomie_ordinaux.lemme4_segments.ensembles_segments_construction import _R_de
...
def bon_ordre_intervalle_depuis_realisation(a="a", Ro="Ro", S="S"):
    ...
    terme de segment NE PORTAIT PAS l'ordre (`seg_ext(a,t)`) : `realisation` était donc
    « Ro-indépendante » et survivait à la généralisation.  C'ÉTAIT UN ARTEFACT DU DÉFAUT
    — le même défaut rendait la théorie CONTRADICTOIRE (⊢ ∅∈∅ ; cf.
    docs/journal/ANOMALIES.md, 2026-07-31).  Le segment porte maintenant son graphe,
    `realisation_hypothese(Ro,a)` est Ro-DÉPENDANTE, et le noyau REFUSE la
    généralisation (C27 : « 'Ro' libre dans une hypothèse » — MESURÉ).

    ⇒ L'HYPOTHÈSE EST DÉSORMAIS Ro-CLOSE : `realisation_hypothese_close(a)` =
        (∃Ro)( bo_form(Ro,a) ∧ (∀c) realisation_segment(Ro,a,c) ),
    « il existe un bon ordre sur a dont les segments réalisent tous les cardinaux ≤ a ».
    C'est l'IDIOME DÉJÀ EN PLACE dans le dépôt (`hyp_transport_ordinal`, E III.24) : le
    bon ordre et la propriété demandée vivent sous LE MÊME ∃.  L'élimination du ∃Ro
    redevient LÉGITIME (plus aucune hypothèse résiduelle où Ro est libre).

    ⚠️ CONSÉQUENCE ASSUMÉE, à ne pas gommer : ZERMELO SEUL NE SUFFIT PLUS.  Zermelo
    donne un bon ordre, PAS la réalisation des segments par ce bon ordre — les deux
    doivent donc être demandées ENSEMBLE.  L'ancien énoncé (hypothèse Ro-libre +
    Zermelo) était STRICTEMENT PLUS FORT et n'était démontrable que par le défaut.
    NOTE : le capstone `bon_ordre_intervalle_close` (ensembles_gate_onto_top, CLOS,
    0 hyp) est INTACT — il décharge sa garde AVANT de généraliser ; ce module reste une
    réduction intermédiaire, plus faible, et c'est mesuré (test_gate_onto_top VERT).
    """
    bo   = BOIO._bo_form_canon(a, Ro, _TPB, _BM, _BX)       # bo_form(Ro,a) bien formé
    real = realisation_hypothese(Ro, a)                     # (∀c) realisation_segment(Ro,a,c)
    boi  = bon_ordre_intervalle_sous_bo_form(Ro, a, S)      # [bo_form, real] ⊢ boi
    assert set(boi.hypotheses) == {bo, real}, "PIÈCE C : hypothèses inattendues"

    # (1) TOUT passer en ANTÉCÉDENT : `real` AUSSI, pas seulement `bo_form`.
    #     C'est LE point de la réparation : `real` porte Ro (le segment porte son
    #     graphe depuis la migration seg_ext), donc on ne peut PAS la laisser en
    #     hypothèse résiduelle pendant la généralisation sur Ro (C27).
    imp1 = N.loi_deduction(bo, boi)                         # bo_form ⇒ boi        [real]
    imp2 = N.loi_deduction(real, imp1)                      # real ⇒ (bo_form ⇒ boi)  CLOS
    ...
    # (2) le ∃Ro est MAINTENANT éliminable : plus AUCUNE hypothèse où Ro est libre.
    ex_imp = existe_elimination(imp_conj, Ro)                # (∃Ro)conj ⇒ boi  CLOS
    H = realisation_hypothese_close(a, Ro)
    res = N.modus_ponens(N.assume(H), ex_imp)                # boi   [ (∃Ro)conj ]

    assert res.conclusion == bon_ordre_intervalle(a),         "conclusion ≠ bon_ordre_intervalle(a) déposé"
...
def realisation_hypothese_close(a="a", Ro="Ro"):
    """L'HYPOTHÈSE, sous sa forme Ro-CLOSE (aucune variable Ro libre) :

        (∃Ro)( bo_form(Ro,a)  ∧  (∀c) realisation_segment(Ro,a,c) )

    « il existe un bon ordre Ro sur a dont les segments réalisent tous les cardinaux
    c ≤ a ».  MÊME IDIOME que `hyp_transport_ordinal` (E III.24) : le bon ordre et la
    propriété qu'on lui demande sont sous LE MÊME ∃."""
    bo = BOIO._bo_form_canon(a, Ro, _TPB, _BM, _BX)
    return existe(Ro, et(bo, realisation_hypothese(Ro, a)))
...
def bon_ordre_intervalle_depuis_realisation_hypotheses(a="a", Ro="Ro"):
    ...
    { (∀c) realisation_segment(Ro,a,c) }, avec Ro LIBRE.  Cette forme-là n'est PLUS
    suffisante — cf. la docstring de `bon_ordre_intervalle_depuis_realisation`."""
    return {realisation_hypothese_close(a, Ro)}
