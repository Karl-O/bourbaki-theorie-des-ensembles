...
import bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_4_entiers_finis.ordinal_cardinal.realisation_segment.ensembles_realisation_segment_close as RSC
import bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_4_entiers_finis.ordinal_cardinal.realisation_segment.ensembles_subset_realise_close as SC
...
def _bon_ordre_intervalle_close_raw(a, Ro="Ro", S="S"):
    ...
    ✅ SURVIT À LA MIGRATION seg_ext (2026-07-31) — MESURÉ, test_gate_onto_top VERT.
    L'élimination du `(∃Ro)` reste LÉGITIME ICI parce que `realisation_segment_garde_clean`
    est DÉCHARGÉE AVANT (étape 2, via `realisation_garde_clean` dont l'unique hypothèse est
    `bo_clean`), puis `bo_clean` est passée en ANTÉCÉDENT : au moment de la généralisation il
    ne reste AUCUNE hypothèse où Ro est libre.  C'est le BON patron — par contraste avec
    `bon_ordre_intervalle_depuis_subset` / `_depuis_realisation`, qui gardaient une
    hypothèse Ro-paramétrée et ne tenaient QUE par le défaut (cf. ANOMALIES.md 2026-07-31).
    """
...
# @livre Ch.III §3.2 Th.1 | E III.24 L.13-15 | PDF p.127
