...
CLOS, 0 hyp :
  ...
  • caractérisations membership `membre_source` (axiome_applications) et
    `membre_but` (axiome_produit_fam : F∈∏famF ⇔ F ⊂ I×⋃famF(ι) ∧ F fonct ∧ dom F=I
                  ∧ (∀ι)(ι∈I⇒F(ι)∈famF(ι))) ;
...
def membre_but(famF="famF", i="I", ff="F"):
    """⊢ ( F ∈ ∏_{ι∈I} 𝓕(B;A_ι) )
         ⇔ ( F ⊂ I × ⋃_ι famF(ι)  ∧  F fonctionnel  ∧  dom F = I
             ∧ (∀ι)(ι∈I ⇒ F(ι) ∈ valeur_famille(famF,ι)) ).
    ...
    ⚠️ Le conjoint de TÊTE « F ⊂ I × ⋃X_ι » a été rétabli le 26 juil. 2026 : la
    CONCLUSION de cette fonction a donc CHANGÉ, sans que rien ne casse (elle est
    seulement instanciée, jamais décomposée ici).  C'est le cas d'école de la dérive
    silencieuse : le test associé compare désormais à une cible reconstruite."""
