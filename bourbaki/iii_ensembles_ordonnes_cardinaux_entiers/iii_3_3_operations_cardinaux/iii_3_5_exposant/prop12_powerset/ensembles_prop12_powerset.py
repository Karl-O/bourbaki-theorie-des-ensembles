...
# (la fonction caractéristique f_Y du livre : « pour toute partie Y de X, soit f_Y
#  l'application de X dans 2, définie par f_Y(x)=α pour x∈Y et f_Y(x)=β pour x∈X−Y ».)
# @livre Ch.III §3.5 Demo.12 | E III.29 L.21-23 | PDF p.132
def chi(y="Y", x="X"):
...
def reunion_Y_diff_egale_X(y="Y", x="X"):
    ...
    from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import cas, tiers_exclu
    vy, vx = _t(y), _t(x)
    vz = var("z")
    diff = E.difference(vx, vy)
