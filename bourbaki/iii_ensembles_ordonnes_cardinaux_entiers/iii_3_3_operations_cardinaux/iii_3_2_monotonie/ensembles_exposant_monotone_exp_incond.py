...
def support_le_image(kappa="kappa", c="C", d="D"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.somme_produit_bornes.ensembles_eq_exposant_invariant import injection_via_pointmap
...
def _R_inclus(a0, s, d, a):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.somme_produit_bornes.ensembles_eq_exposant_invariant import _membre_graphe_terme_z
...
def _ghat_fonctionnel(vg, va0, vs, vd, va):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.recollement.ensembles_restriction_somme import reunion_graphes_fonctionnelle
...
def _produit_mono_gauche(s, d, a):
    ...
    from bourbaki.ii_theorie_des_ensembles.ii_2_couples.ii_2_2_produit_deux_ensembles.ensembles_produit import produit_inclusion_facile
...
def _ghat_coincide_S(vg, va0, vs, vd, va, s_nom):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.recollement.ensembles_recollement_bijection import valeur_reunion_gauche
    grg = graphe_de(vg)
    R = R_terme(va0, vs, vd)
    vs_pt = var(s_nom)
...
# (injection (C<=D, A non vide) => F(C;A) <= F(D;A) — démo du Cor.2, volet exposant ;
#  la condition a'>0 du livre = la valeur-défaut a0 dans A pour prolonger à D.)
# @livre Ch.III §3.6 Demo.- | E III.30 L.18-19 | PDF p.133
...
# ═══════════════════════════════════════════════════════════════════════════════
# (Cor.2 §3.6, volet EXPOSANT, INCONDITIONNEL : (c<=d, a≠0) => (a^c <= a^d).)
# @livre Ch.III §3.6 Cor.2 | E III.30 L.16-17 | PDF p.133
