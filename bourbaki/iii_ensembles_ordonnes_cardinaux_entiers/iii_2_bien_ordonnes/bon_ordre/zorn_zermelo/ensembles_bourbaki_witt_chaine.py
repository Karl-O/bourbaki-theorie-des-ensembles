...
def Mc_clos_sup(G="G", E_set="E", p="p", a="a", c="c", C="C", s="s",
    ...
    # ¬(∀x)R = ¬¬(∃x)¬R → (∃x)¬R   (dne)
    from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import dne as _dne
...
def _est_ordre_M(G, E_set, p, a, x="x", y="y", z="z"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_1_relations_ordre.ordre_treillis.ensembles_ordre_relation import ordre_induit_sur_partie
    vG, vE, vp, va = _terme(G), _terme(E_set), _terme(p), _terme(a)
    Mt = M(vG, vE, vp, va)
...
# @livre Ch.III §2.3 Demo.3 | E III.19 L.20-38 | PDF p.122  (démonstration du Lemme 3 — route du dépôt : point fixe de Bourbaki–Witt sans choix, « M est une chaîne »)
# @livre Ch.III §2.3 Demo.3 | E III.20 L.1-5 | PDF p.123  (fin de la démonstration du Lemme 3)
