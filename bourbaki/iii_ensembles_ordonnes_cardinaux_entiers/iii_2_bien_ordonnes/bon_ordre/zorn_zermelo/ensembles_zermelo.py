...
def _frais(*termes, base="w"):
    ...
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import libres_t, _fraiche
    evite = set()
    for t in termes:
        evite |= libres_t(t)
...
def Theta_reflexive_sur(X="X", G="x"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_1_relations_ordre.ordre_treillis.ensembles_ordre_relation import reflexivite_sur
    vX, vG = var(X), var(G)
...
def _doubleton_non_vide(u, v):
    ...
    from bourbaki.ii_theorie_des_ensembles.ii_1_relations_collectivisantes.ensembles_theoremes import appartient_paire_gauche, vide_sans_element
...
def Theta_transitive(X="X", G="x", H="y", K="z"):
    ...
    if Ht != H:
        _, inner = __import__("bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2",
...
def Union_majorant(X="X", D="D", G="x"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_1_relations_ordre.ordre_treillis.ensembles_ordre_relation import totalement_ordonne, majorant
    vX, vD = var(X), var(D)
    Ut = Union(vX, vD)
    HDW = N.assume(inclus(vD, W(vX)))                          # 𝔇⊂W
...
def W_inductif(X="X", D="C", m="m", x="x", y="y", z="z"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.zorn_zermelo.ensembles_zorn import chaine, est_inductif
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_1_relations_ordre.ordre_treillis.ensembles_ordre_relation import totalement_ordonne, majorant
    vX, vD = var(X), var(D)
...
def _Ext_bien_ordonne_corps(X, M, x0, GW, Hx0nd, S="S", aa="a", ww="w", s0="s0", m="mw"):
    ...
    if "se" != ww:
        _, inner2 = __import__("bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2",
...
# @livre Ch.R §6 Th.- | E.R.27 item 5 (théorème de Zermelo : tout ensemble peut être bien ordonné) | PDF p.330
# @livre Ch.III §2.3 Demo.1 | E III.20 L.10-15 | PDF p.123  (démonstration du Th. 1 — livre : lemme 3 + p(X)=τ_x(x∈E∖X) ; ici via Zorn, écart de route documenté)
