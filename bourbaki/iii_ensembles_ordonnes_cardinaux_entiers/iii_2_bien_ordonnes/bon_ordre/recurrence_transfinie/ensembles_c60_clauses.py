...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_existence_close import est_essai, couvert_essai, dom_essai
...
def valeur_membre_egale_regle(vh, e="E", G="G", x="x0", V="Vval",
    ...
    corps_y = et(appartient(vy, seg), est_essai(vp, vh, G, ve, vy, z))
...
def _couverture_membre(vh, e, G, x, V, z, p="pcf", yD="yD", zb="zess"):
    ...
    # corps du témoin p :  p∈amb ∧ est_essai(p, vh, G, E, z)
    corps_p = et(appartient(vp, ambiant(e, V)), est_essai(vp, vh, G, ve, vz, zb))
    ...
    sel_corps = et(appartient(vz, seg), est_essai(vp, vh, G, ve, vz, zb))
    sel_zz = conjonction_intro(h_zseg, essai_p)                   # z∈seg ∧ est_essai(p,z)
    sel_ex = N.modus_ponens(sel_zz, N.s5(
        et(appartient(var(yD), seg), est_essai(vp, vh, G, ve, var(yD), zb)), vz, yD))  # (∃yD)(...)
    ...
    segz = E.segment_extremite(_t(G), ve, vz)
    ...
    dom_eq_sym = N.modus_ponens(dom_eq, symetrie(E.dom(vp), dom_essai(G, ve, vz)))  # seg(z)∪{z}=dom(p)
    z_in_domp = N.modus_ponens(z_in_dom_essai, equivalence_avant(
        N.modus_ponens(dom_eq_sym, N.s6(dom_essai(G, ve, vz), E.dom(vp), "wdz", appartient(vz, var("wdz"))))))  # z∈dom(p)
...
def couverture_segment_realise(vh, e="E", G="G", x="x0", V="Vval", y="ytf",
    ...
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import inclus, non, ou
    from bourbaki.ii_theorie_des_ensembles.ii_1_relations_collectivisantes.ensembles_theoremes import _instance_reunion
    from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import cas, antecedent_consequent
    R = _graphe_R(G)
    ve, vx = _t(e), var(x)
    Dx = Dfam_real(vh, e, G, vx, V)
    Ux = union_famille(Dx)
    ...
    vys = var("yseg")
    corps_ys = et(appartient(vys, seg), est_essai(vpun, vh, G, ve, vys, zb))
    ...
    segys = E.segment_extremite(_t(G), ve, vys)
    sys = E.singleton(vys)
    z_in_segys_union = N.modus_ponens(z_in_dpun, equivalence_avant(
        N.modus_ponens(dom_pun_eq, N.s6(E.dom(vpun), dom_essai(G, ve, vys), "wdz", appartient(vz, var("wdz"))))))
    z_disj = N.modus_ponens(z_in_segys_union, equivalence_avant(_instance_reunion(segys, sys, vz)))  # z∈seg(yseg) ou z∈{yseg}

    # segment membership instances (binder order G, E, x, y ; axiome CLOS) :
    th_seg = E.theorie_segment_extremite()
    ax_seg = instancie(N.axiome(th_seg, E.axiome_segment_extremite()), _t(G))
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §2.2 Demo.60 | E III.18 L.34-39 | PDF p.121  (démonstration de C60, existence : famille des segments couverts par un essai — clauses P2-P4)
# @livre Ch.III §2.2 Demo.60 | E III.19 L.1-5 | PDF p.122  (fin de la démonstration de C60 : recollement des essais + prolongement d'un pas)
