...
def point_hors_segment(G="G", e="E", x="x0"):
    ...
    ax = instancie(N.axiome(th, E.axiome_segment_extremite()), _t(G))
...
def dom_essai(G, e, x):
    ...
    return E.reunion(E.segment_extremite(_t(G), _t(e), _t(x)), E.singleton(_t(x)))


def est_essai(p, vh, G, e, x, z="zess"):
...
def couvert_essai(vh, G, e, p="pess", z="zess"):
    ...
    return lambda x: existe(p, est_essai(var(p), vh, G, e, x, z))


# @livre Ch.III §2.2 Demo.60 | E III.18 L.34-39 | PDF p.121  (démonstration de C60, existence : tout segment est couvert par un essai — couverture via C59/lemme 2)
...
def couverture_essais_via_c59(vh, e="E", G="G", x0="x0tf", y="ytf",
    ...
    R = _graphe_R(G)
    ve = _t(e)
    couvert = couvert_essai(vh, G, ve, p, z)
