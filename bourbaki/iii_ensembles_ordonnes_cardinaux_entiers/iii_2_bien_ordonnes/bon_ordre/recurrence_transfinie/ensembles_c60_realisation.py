...
LA CONSTRUCTION (S8) — la FAMILLE CONCRÈTE des essais des y<x.
  ...
  Dfam_real(x) := { p ∈ 𝔓(E×V) | (∃y)( y∈seg(R,E,x) ∧ est_essai(p, vh, G, E, y) ) }
...
def Dfam_real(vh, e="E", G="G", x="x0", V="Vval"):
    ...
    vh : règle Terme→Terme (lue dans est_essai).  Le terme INDEX `c60_Dfam_real` ne
    porte que E, x, V (comme `Ncol(a)` n'embarque pas l'ordre — la relation
    R=(·,·)∈G et la règle vh sont CAPTURÉES dans le sélecteur de l'axiome ci-dessous).
    ⚠️ CORRIGÉ (migration seg_ext) : le SEGMENT, lui, est désormais seg_ext(G,E,x) et
    PORTE le graphe — il n'est plus indépendant de G au niveau terme.  Ne pas réutiliser
    l'ancienne remarque pour éliminer un ∃G."""
...
def _corps_Dfam_real(vh, e, G, x, p, V="Vval", y="yD"):
    ...
    sel = existe(y, et(appartient(vy, seg), est_essai(vp, vh, G, _t(e), vy)))
    return et(amb, sel)
...
def membres_fonctionnels_realise(vh, e="E", G="G", x="x0", V="Vval",
    ...
    vy = var(y)
    corps_y = et(appartient(vy, seg), est_essai(vp, vh, G, ve, vy))
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §2.2 Demo.60 | E III.18 L.34-39 | PDF p.121  (démonstration de C60, existence : réalisation par S8 de la famille concrète des essais)
# @livre Ch.III §2.2 Demo.60 | E III.19 L.1-5 | PDF p.122  (fin de la démonstration de C60)
def recursion_transfinie_existence_reduite(vh, e="E", G="G", V="Vval",
    ...
    vh produit un terme vh(x) — la valeur-règle réifiée comme terme dépendant.  Le
    choix vval:=vh est canonique (f(x)=h(x,f|seg)).

    ⚠️ CETTE LIGNE A DIT « vh DOIT … SANS τ interne (sinon collision de capture avec les
    liants de dom_singleton_couple du noyau déposé) ».  C'EST PÉRIMÉ depuis le fix
    `subst` du 24 juil. 2026.  MESURÉ le 26 juil. :
        recursion_transfinie_existence_reduite(app('rule',·))      → 4 hyps, 0,8 s
        recursion_transfinie_existence_reduite(regle_factorielle()) → 4 hyps, 1,1 s
    alors que `regle_factorielle()` est τ-LOURDE (binders F,Z,Zfac62,u,up,v,y,yfac62,z,
    dont u,v,y,z homonymes des liants du gluing).  Une règle τ-lourde passe.  Un lecteur
    qui prend cet avertissement au mot renonce à un chemin qui fonctionne.
