...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.recollement.ensembles_restriction_somme import antecedent_dans_domaine
...
from bourbaki.ii_theorie_des_ensembles.ii_5_produit_famille.ii_5_definitions.ensembles_produit_famille import _inst_parties
...
def essai_dans_parties(vh, e="E", G="G", y="ypont", V="Vval",
    ...
    # ── hypothèses honnêtes ──────────────────────────────────────────────────
    h_essai = N.assume(est_essai(vp, vh, G, ve, vy, z))          # est_essai(p,y)
    ...
    assert est_essai(vp, vh, G, ve, vy, z) in res.hypotheses, "essai_dans_parties : est_essai absente"
...
def essai_dans_parties_depuis_bien_formes(vh, e="E", G="G", y="ypont", V="Vval",
    ...
    h_essai = est_essai(vp, vh, G, ve, vy, z)
...
def antecedent_ambiant_depuis_bare(vh, e="E", G="G", x="x0", V="Vval",
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_realisation import antecedent_couverture
    ...
    essai_pess = est_essai(vpess, vh, G, ve, vy, z)
    ...
    gabarit = et(appartient(var(p), ambiant(e, V)), est_essai(var(p), vh, G, ve, vy, z))
...
# @livre Ch.III §2.2 Demo.60 | E III.18 L.34-39 | PDF p.121  (démonstration de C60, existence : pont bare→ambiant P(E×V) des essais)
...
def recursion_transfinie_existence_final(vh, e="E", G="G", V="Vval",
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_clauses import (
    ...
    seg_b = lambda vx: E.segment_extremite(_t(G), ve, vx)
    def _conc3(vx):
        Ux = union_famille(Dfam_real(vh, e, G, vx, V))
        return egal(E.dom(Ux), E.segment_extremite(_t(G), ve, vx))
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_final import recursion_sur_segment
    def _conc4(vx):
