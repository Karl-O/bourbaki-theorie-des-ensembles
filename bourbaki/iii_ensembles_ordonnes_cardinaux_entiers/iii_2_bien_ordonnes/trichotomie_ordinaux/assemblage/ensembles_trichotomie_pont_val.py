...
from bourbaki.ii_theorie_des_ensembles.ii_3_correspondances.ii_3_4_fonctions.ensembles_valeur_codomaine import valeur_dans_codomaine
...
# @livre Ch.III §2.5 Demo.3 | E III.21 L.26-29 | PDF p.124  (démonstration du Th. 3 : les valeurs d'un iso de segments tombent dans F — v(S) segment de F)
...
# ════════════════════════════════════════════════════════════════════════════
#  (4)  temoin_surjectif_dans_S — le maillon de temoin_dans_S, DÉRIVÉ.
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §2.5 Demo.3 | E III.21 L.26-29 | PDF p.124  (démonstration du Th. 3 : la surjectivité de l'iso témoin φ:S≅T fournit l'antécédent p∈S de u — le pendant image du codomaine)
def temoin_surjectif_dans_S(E_set="E", R="R", F_set="F", Rp="Rp",
                            u="ua", t="ta", x="x", S="S", T="T", phi="phi",
                            pw="pz", w="qim"):
    """{ φ⊂S×T, dom φ=S, func φ, x∈S, t=φ(x), seg(S,R,E), seg(T,Rp,F),
        iso(φ,S,T,R,Rp,"px","pw"), u∈F, Rp{u,t} }
      ⊢ (∃pz)( (pz∈S et pz∈E) et valeur(φ,pz)=u ).                 [10 hyps]

    DÉRIVE le maillon que temoin_dans_S (ensembles_trichotomie_img_segment)
    postulait : t=φ(x)∈T [valeur_iso_dans_T + transport S6 le long de t=φ(x)] ;
    u∈T [initialité du segment T instanciée (t,u)] ; u∈φ⟨S⟩ [est_surjective(φ,S,T)
    = (φ⟨S⟩=T) via iso→bijective→surjective, transport S6 trou côté ensemble] ;
    ∃-témoin par AXIOME_IMAGE — RENOMMÉ « qim » (« x » est PRIS, libre dans
    t=φ(x) : S5 exotique puis élimination sur « x ») ; φ(qim)=u
    [couple_donne_valeur + func φ] ; qim∈E [S⊂E instancié].
    theorie=22, rien postulé, NON vacueux."""
    ...
    Rf, Rpf = _R_de(R), _R_de(Rp)
    vE, vF = _t(E_set), _t(F_set)
    vu, vt, vx, vw = var(u), var(t), var(x), var(w)
    vS, vT, vphi = _t(S), _t(T), _t(phi)
    phi_x = E.valeur(vphi, vx)

    # (a) φ(x)∈T  [φ⊂S×T, dom φ=S, x∈S]     (b) t∈T  [+ t=φ(x), transport S6]
    phix_in_T = valeur_iso_dans_T(phi, S, T, x)
    Ht_eq = N.assume(egal(vt, phi_x))
    tr1 = N.s6(vt, phi_x, "h6t", appartient(var("h6t"), vT))
    t_in_T = N.modus_ponens(phix_in_T,
                            equivalence_arriere(N.modus_ponens(Ht_eq, tr1)))

    # (c) u∈T  [initialité du segment T, instanciée (t,u)]
    Hseg_T = N.assume(E.est_segment(vT, Rpf, vF))
    init_inst = instancie(instancie(conjonction_elim_droite(Hseg_T), vt), vu)
    Hu_in_F = N.assume(appartient(vu, vF))
    HRut = N.assume(Rpf(vu, vt))
    prem = conjonction_intro(conjonction_intro(t_in_T, Hu_in_F), HRut)
    u_in_T = N.modus_ponens(prem, init_inst)

    # (d) est_surjective : φ⟨S⟩=T  [iso → bijective → surjective]
    Hiso = N.assume(V.est_isomorphisme_ordre(vphi, vS, vT, Rf, Rpf, "px", "pw"))
    Hsurj = conjonction_elim_droite(conjonction_elim_gauche(Hiso))

    # (e) u∈φ⟨S⟩  [transport S6 le long de φ⟨S⟩=T, trou CÔTÉ ENSEMBLE]
    tr2 = N.s6(E.image(vphi, vS), vT, "h6s", appartient(vu, var("h6s")))
    u_in_img = N.modus_ponens(u_in_T,
                              equivalence_arriere(N.modus_ponens(Hsurj, tr2)))

    # (f) ∃x(x∈S et (x,u)∈φ)  [AXIOME_IMAGE instancié G:=φ, X:=S, y:=u]
    ax_img = N.axiome(E.theorie_ensembles(), E.AXIOME_IMAGE)
    car = instancie(instancie(instancie(ax_img, vphi), vS), vu)
    ex_x = N.modus_ponens(u_in_img, equivalence_avant(car))

    # (g) témoin RENOMMÉ « x »→« qim » (x est libre dans t=φ(x) : S5 exotique)
    body_w = et(appartient(vw, vS), appartient(E.couple(vw, vu), vphi))
    ex_w = N.modus_ponens(ex_x, existe_elimination(N.s5(body_w, vx, w), x))

    # (h) sous le témoin qim : φ(qim)=u, qim∈E, corps du ∃pz
    Hw = N.assume(body_w)
    Hw_in_S = conjonction_elim_gauche(Hw)
    Hw_couple = conjonction_elim_droite(Hw)
    Hfunc = N.assume(E.est_fonctionnel(vphi))
    cdv = couple_donne_valeur(vphi, vw, vu)          # {func φ, (qim,u)∈φ} ⊢ u=φ(qim)
    for hyp_f, preuve_h in [(appartient(E.couple(vw, vu), vphi), Hw_couple),
                            (E.est_fonctionnel(vphi), Hfunc)]:
        cdv = N.modus_ponens(preuve_h, N.loi_deduction(hyp_f, cdv))
    val_eq_u = N.modus_ponens(cdv, symetrie(vu, E.valeur(vphi, vw)))   # φ(qim)=u
    Hseg_S = N.assume(E.est_segment(vS, Rf, vE))
    w_in_E = N.modus_ponens(Hw_in_S,
                            instancie(conjonction_elim_gauche(Hseg_S), vw))
    corps_pz = et(et(appartient(var(pw), vS), appartient(var(pw), vE)),
                  egal(E.valeur(vphi, var(pw)), vu))
    corps_w = conjonction_intro(conjonction_intro(Hw_in_S, w_in_E), val_eq_u)
    ex_pz = N.modus_ponens(corps_w, N.s5(corps_pz, vw, pw))

    # (i) éliminer le témoin qim (absent des hyps restantes et de la conclusion)
    imp_w = N.loi_deduction(body_w, ex_pz)
    res = N.modus_ponens(ex_w, existe_elimination(imp_w, w))

    cible = existe(pw, corps_pz)
    assert res.conclusion == cible, "temoin_surjectif_dans_S : conclusion ≠ ∃pz(corps)"
    assert len(res.hypotheses) == 10, \
        "temoin_surjectif_dans_S : hyps ≠ 10 (%d)" % len(res.hypotheses)
    assert res.conclusion not in res.hypotheses, "temoin_surjectif_dans_S : VACUOUS"
    return res
