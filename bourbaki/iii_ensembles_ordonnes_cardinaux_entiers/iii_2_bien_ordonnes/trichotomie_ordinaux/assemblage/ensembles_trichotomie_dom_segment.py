...
def dom_h_initial_sous_val(E_set="E", R="R", F_set="F", Rp="Rp",
    ...
    # ── φ(y)∈F : par l'HYPOTHÈSE val_dans_F (défaut) ou PAR LE PONT (via_pont) ──
    if via_pont:
        # Recâblage du 2 août 2026 : le φ-TÉMOIN porte la structure {φ⊂S×T, dom φ=S}
        # (conjoints « φ APPLICATION » du cœur, pelés ci-dessus) — `valeur_iso_dans_F`
        # (pont_val, CLOS sous 4 hyps) remplace l'hypothèse val_dans_F, qui MEURT.
        from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.trichotomie_ordinaux.assemblage.ensembles_trichotomie_pont_val import valeur_iso_dans_F
        vif = valeur_iso_dans_F(phi, S, T, F_set, Rp, y)
        for hyp_f, pr in [(inclus(vphi, E.produit(vS, vT)), Hgraph),
                          (egal(E.dom(vphi), vS), Hdom),
                          (appartient(vy, vS), Hy_in_S),
                          (E.est_segment(vT, Rpf, vF), Hseg_T)]:
            vif = N.modus_ponens(pr, N.loi_deduction(hyp_f, vif))
        Hphi_y_in_F = vif                                     # φ(y)∈F  [sous le cœur]
    else:
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §2.5 Demo.3 | E III.21 L.26-29 | PDF p.124  (démonstration du Th. 3 : « la réunion S des ensembles de définition … est un segment de E » — dom(h) segment)
...
# ════════════════════════════════════════════════════════════════════════════
#  VERSIONS PROUVÉES (2 août 2026) — val_dans_F MORTE, dérivée par le pont.
# ════════════════════════════════════════════════════════════════════════════
def dom_h_initial_prouve(E_set="E", R="R", F_set="F", Rp="Rp",
                         x="xa", y="ya", S="S", T="T", phi="phi"):
    """⊢ (∀x)(∀y)( (x∈dom h et y∈E et R{y,x}) ⇒ y∈dom h ).            [CLOS]"""
    res = dom_h_initial_sous_val(E_set, R, F_set, Rp, x, y, S, T, phi,
                                 via_pont=True)
    assert res.conclusion == dom_h_initial_cible(E_set, R, F_set, Rp, x, y), \
        "dom_h_initial_prouve : conclusion inattendue"
    assert res.est_clos, "dom_h_initial_prouve : hypothèses résiduelles"
    return res


def dom_h_est_segment_prouve(E_set="E", R="R", F_set="F", Rp="Rp",
                             x="xa", y="ya", S="S", T="T", phi="phi"):
    """⊢ est_segment(dom h, R, E).                                     [CLOS]"""
    incl = M.h_dom_inclus_E(E_set, R, F_set, Rp)
    init = dom_h_initial_prouve(E_set, R, F_set, Rp, x, y, S, T, phi)
    res = conjonction_intro(incl, init)
    assert res.conclusion == dom_h_est_segment_cible(E_set, R, F_set, Rp, x, y), \
        "dom_h_est_segment_prouve : ≠ cible"
    assert res.est_clos, "dom_h_est_segment_prouve : non clos"
    return res
...
__all__ = [
    ...
    "dom_h_initial_prouve", "dom_h_est_segment_prouve",
]
