...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.3 Demo.2 | E III.49 L.1-2 | PDF p.152  (« Il y a donc une application bijective f₁ de Y sur (F×Y)∪(Y×F)∪(Y×Y) »)
...
def cadre_ensemble_reunion(S="S0", U="Ucadre"):
    """Le CADRE-RÉUNION F_r := (S₀×U) ∪ ( (U×S₀) ∪ (U×U) )  (E.III.48, Z²∖S₀² LITTÉRAL).

    Variante NON taguée du cadre : sous U∩S₀=∅ les trois produits sont deux à
    deux disjoints (ensembles_produits_disjoints), F_r est équipotent au cadre
    tagué (cadre_card_trois_b_reunion) et S₀²∪F_r = Z² est un THÉORÈME CLOS
    (s0sq_cadre_reunion_egale_carre) — c'est la clef du déblocage de STEP B2."""
    vS, vU = _t(S), _t(U)
    return E.reunion(E.produit(vS, vU),
                     E.reunion(E.produit(vU, vS), E.produit(vU, vU)))


# @livre Ch.III §6.3 Demo.2 | E III.48 L.31-37 | PDF p.151  (Z×Z=(F×F)∪(F×Y)∪(Y×F)∪(Y×Y) disjoints, Card du cadre = 3𝔟 = 𝔟)
def cadre_card_trois_b(S="S0", U="Ucadre"):
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_6_infinis.hessenberg.coeur.ensembles_descentes_inconditionnelles import _bien_definie_t
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.3 Demo.2 | E III.49 L.3-4 | PDF p.152  (« l'application g de Z dans Z×Z égale à f dans F et à f₁ dans Y est une bijection qui prolonge f »)
def phi_etendue_bijection(phi0="phi0", psi="psi", S="S0", U="Ucadre",
    ...
    via_reunion=True : le cadre F est pris en RÉUNION (cadre_ensemble_reunion) —
    l'hyp S₀²∪F=Z² du GAP-A dom devient alors le THÉORÈME CLOS
    s0sq_cadre_reunion_egale_carre (déblocage de STEP B2, cf. journal).
    ...
    Fcadre = (cadre_ensemble_reunion(S, U) if via_reunion
              else cadre_ensemble(S, U))                    # F = cadre Z²∖S₀²
    SxS = E.produit(vS, vS)
...
def projection_seconde_t(ta, tb):
    ...
    from bourbaki.ii_theorie_des_ensembles.ii_2_couples.ii_2_1_definition_couples.ensembles_projections import projection_seconde
...
# @livre Ch.III §6.3 Demo.2 | E III.49 L.3-4 | PDF p.152  (« … ce qui est contraire à la définition de f » : contradiction avec la maximalité)
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.3 Demo.2 | E III.48 L.25-26 | PDF p.151  (« Montrons que Card(F) = 𝔞, ce qui démontrera le théorème »)
def card_S0_egal_card_E(S="S0", E_set="E"):
    ...
    from bourbaki.i_description_mathematique_formelle.i_2_theoremes.tactiques.tactiques_abrege2 import tiers_exclu, cas
...
# @livre Ch.III §6.3 Demo.2 | E III.49 L.1-4 | PDF p.152  (fin de la démonstration : 𝔟=𝔞 par maximalité, d'où a²=a)
