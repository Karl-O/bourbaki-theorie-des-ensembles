...
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import (Terme, var, app, tau, egal, et, non, impl, equiv, ou,
                     existe, pourtout, appartient, inclus)
from bourbaki.i_description_mathematique_formelle.i_2_theoremes.noyau import noyau_abrege as N
from bourbaki.ii_theorie_des_ensembles.ii_1_relations_collectivisantes import ensembles_abrege as E
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.definitions_cardinaux.ensembles_cardinaux import cardinal, equipotent, inf_egal_card
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_4_entiers_finis.iii_4_1_definitions_premiers_entiers.ensembles_entiers import est_fini, est_fini_ensemble, est_entier
...
# @livre Ch.III §6.1 Def.- | E III.45 L.21-22 | PDF p.148  (« Nous désignerons par N l'ensemble des entiers … Le cardinal de N se note aussi ℵ₀ »)
def aleph0():
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.1 Rem.- | E III.45 L.1-2 | PDF p.148  (titres « §6. ENSEMBLES INFINIS » / « 1. L'ensemble des entiers naturels » — prose, rien à formaliser)
...
# @livre Ch.III §6.1 Def.1 | E III.45 L.3-4 | PDF p.148  (forme ensembliste : E infini := ¬Fini(Card E))
...
# @livre Ch.III §6.1 Ax.A4 | E III.45 L.14-15 | PDF p.148  (« A4 (« axiome de l'infini »). Il existe un ensemble infini. »)
# @livre Ch.III §6.1 Rem.- | E III.45 L.16-18 | PDF p.148  (petit texte : indépendance présumée de A4 — prose, rien à formaliser)
def theorie_infini():
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.1 Def.2 | E III.45 L.25-27 | PDF p.148
# @livre Ch.III §6.1 Rem.- | E III.45 L.28-35 | PDF p.148  (notations (x_n), ∏ etc. — prose, rien à formaliser)
# @livre Ch.III §6.1 Rem.- | E III.45 L.36-37 | PDF p.148  (sous-famille d'une suite = suite « extraite » — prose, rien à formaliser)
# @livre Ch.III §6.1 Rem.- | E III.46 L.1-3 | PDF p.149  (suites ne différant que par l'ordre des termes — prose, non formalisé)
# @livre Ch.III §6.1 Rem.- | E III.46 L.4-6 | PDF p.149  (suite multiple, suite double — prose, non formalisé)
# @livre Ch.III §6.1 Rem.- | E III.46 L.7-13 | PDF p.149  (rangement d'une famille dans l'ordre défini par une bijection — prose, non formalisé)
def est_suite(f, i):
...
# @livre Ch.III §6.1 Def.2 | E III.45 L.25-27 | PDF p.148  (« la suite est dite infinie si son ensemble d'indices est une partie infinie de N »)
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.4 Def.3 | E III.49 L.20-22 | PDF p.152
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.4 Def.4 | E III.50 L.24-25 | PDF p.153
# @livre Ch.III §6.4 Rem.- | E III.50 L.28-34 | PDF p.153  (petit texte : hypothèse du continu, Gödel–Cohen — prose métamathématique, rien à formaliser)
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.5 Def.5 | E III.50 L.35-37 | PDF p.153
...
# ═══════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.5 Def.- | E III.51 L.22-23 | PDF p.154
