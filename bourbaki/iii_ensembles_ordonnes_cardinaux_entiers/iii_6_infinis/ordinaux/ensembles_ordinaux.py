...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_3_equipotence_cardinaux.definitions_cardinaux import ensembles_cardinaux as C
...
def segment_initial_extremite(e, G, a):
    """segment_initial_extremite(E, R, a) := segment_extremite(G, E, a) = ]←, a[.
    ...
    return E.segment_extremite(_terme(G), _terme(e), _terme(a))
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6 Ex.10 | E III.87 L.44-45 | PDF p.190  (exerc. 10 b) : « on dit que ω_α est l'ordinal initial d'indice α » — la notion vit dans les EXERCICES du §6, pas dans le texte E III.45)
...
# @livre Ch.III §6 Ex.10 | E III.87 L.44-47 | PDF p.190  (exerc. 10 b)-c) : ω_α, ℵ_α = Card(ω_α) ; ω_α = plus petit ordinal de cardinal ℵ_α — exercices du §6)
