...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.3 Demo.2 | E III.49 L.3-4 | PDF p.152  (g = f∪f₁ : Z→Z×Z bijective, dérivée des deux bijections)
def phi1_bijection_derivee(E_set="E", phi0="phi0", psi="psi", S="S0", U="Ucadre",
    ...
    SxS = E.produit(vS, vS)
    if via_reunion:
        from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_6_infinis.frame_zorn.ensembles_frame_extension_finale import cadre_ensemble_reunion
        F = cadre_ensemble_reunion(S, U)                     # le cadre en RÉUNION
    else:
        F = cadre_ensemble(S, U)                             # le cadre (somme disjointe)

    base = phi_etendue_bijection(phi0, psi, S, U,
                                 via_reunion=via_reunion)    # 12 hyps honnêtes ⊢ bij(φ₁,Z²,Z)
...
def extension_dans_frame_chainee(E_set="E", phi0="phi0", psi="psi", S="S0", U="Ucadre",
    ...
    step1 = phi1_bijection_derivee(E_set, phi0, psi, S, U,
                                   via_reunion=via_reunion)  # ⊢ bij
...
def extension_ordre_chainee(E_set="E", phi0="phi0", psi="psi", S="S0", U="Ucadre",
    ...
    step2 = extension_dans_frame_chainee(E_set, phi0, psi, S, U,
                                         via_reunion=via_reunion)  # ⊢ (Z,φ₁)∈𝔉
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.3 Demo.2 | E III.49 L.3-4 | PDF p.152  (la maximalité force Z = F : le lock est DÉRIVÉ, non supposé)
def extension_force_egalite_chainee(E_set="E", phi0="phi0", psi="psi", S="S0", U="Ucadre",
    ...
    step2 = extension_dans_frame_chainee(E_set, phi0, psi, S, U,
                                         via_reunion=via_reunion)   # ⊢ (Z,φ₁)∈𝔉
    step3 = extension_ordre_chainee(E_set, phi0, psi, S, U,
                                    via_reunion=via_reunion)   # ⊢ ((S₀,φ₀),(Z,φ₁))∈Γ𝔉
...
# ════════════════════════════════════════════════════════════════════════════
# @livre Ch.III §6.3 Demo.2 | E III.49 L.3-4 | PDF p.152  (« ce qui est contraire à la définition de f » : contradiction chaînée)
def extension_absurde_chainee(E_set="E", phi0="phi0", psi="psi", S="S0", U="Ucadre",
                              u="uwit", via_reunion=False):
    ...
    step4 = extension_force_egalite_chainee(E_set, phi0, psi, S, U,
                                            via_reunion=via_reunion)   # ⊢ Z=S₀
