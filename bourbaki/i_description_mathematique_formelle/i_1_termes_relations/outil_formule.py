...
from bourbaki.i_description_mathematique_formelle import assemblage as A
...
def subst_t(tval: Terme, x: str, t: Terme) -> Terme:
    if x not in _libres_t(t):
        return t          # (T|x)t = t quand x n'a AUCUNE occurrence libre (CS, E I.31) —
                          # court-circuit qui interdit tout renommage GRATUIT de liant
                          # (fix verrou-τ 2026-07-24 : deux chemins de subst composés
                          # divergeaient sur des α-renommages sans risque de capture).
    if t.tag == "var":
        return tval                       # x libre dans une variable ⇒ t est x
    if t.tag == "tau":
        y, corps = t.lieur, t.args[0]     # y ≠ x (sinon x ne serait pas libre dans t)
...
def subst_f(tval: Terme, x: str, f: Formule) -> Formule:
    if x not in _libres_f(f):
        return f          # (T|x)f = f quand x n'est pas libre (même court-circuit CS).
    if f.tag == "exists":
        y, corps = f.lieur, f.sous[0]     # y ≠ x (sinon x ne serait pas libre dans f)
        if y in libres_t(tval):
            z = _fraiche(libres_t(tval) | libres_f(corps) | {x})
            corps, y = subst_f(var(z), y, corps), z
