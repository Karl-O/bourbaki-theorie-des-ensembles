...
non-vide :
  ...
  La BIJECTIVITÉ de g (Prop. 3) reste REPORTÉE ; on prouve le SENS FACILE
  « g bien définie / compatible ».  [theoremes `cofinal_canonique_coordonnee`,
  `cofinal_canonique_compatible`]

  ── 4.  COFINAL ⇒ CANONIQUE INDUCTIVE : SURJECTIVITÉ « SENS FACILE »  ──
  (Prop. 8 §III.7.7, SENS FACILE.)  L'application canonique f_α : E_α → E=lim→ a pour
...
def cofinal_canonique_coordonnee(Efam="E", f="f", leq=None, i="I", J="J",
    ...
    lue coordonnée par coordonnée.  Instance de l'AXIOME canonique g (déjà posé dans
    `ensembles_limites_canoniques`, théorie dédiée — theorie_ensembles inchangé).
...
def cofinal_canonique_compatible(Efam="E", f="f", leq=None, i="I", J="J",
    ...
    E'=lim←_J (la BIJECTIVITÉ de g, elle, reste REPORTÉE).
    ...
    eq3_a = cofinal_canonique_coordonnee(vE, vf, leq, vi, vJ, x, a)   # pr_α g(x)=f_α(x)
    eq3_b = cofinal_canonique_coordonnee(vE, vf, leq, vi, vJ, x, b)   # pr_β g(x)=f_β(x)
...
def canonique_ind_atteint(Efam="E", f="f", i="I", gleq=None, a="a", x="x"):
    ...
    (§III.7.5/7) : la classe Cl_R(x) de tout x∈E_α est dans l'IMAGE de f_α (à savoir
...
REPORTES = [
    ...
    "(5)) est prouvé.",
    "Corollaire 1 Prop. 1 / Prop. 6 (EXISTENCE/UNICITÉ de lim← u / lim→ u) — REPORTÉ "
    "(seul le SENS FACILE pointwise du diagramme propagé est prouvé).",
    "Proposition 3 (§III.7.2) : application canonique g cofinale BIJECTIVE — seul le "
    "SENS FACILE « g bien définie / coordonnées compatibles » est prouvé.",
    ...
    "Proposition 8 (§III.7.7) : canonique cofinale E'→E BIJECTIVE — seul le SENS "
