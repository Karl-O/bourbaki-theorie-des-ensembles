...
réutilisées ici, AUCUNE modification d'un fichier existant) :
  ...
  • `bourbaki.ordre.ensembles_limites_canoniques` : est_systeme_projectif_parties,
...
#  (III.7) SYSTÈMES relatifs à un ensemble d'indices FILTRANT
# ════════════════════════════════════════════════════════════════════════════
def est_systeme_projectif_filtrant(f, leq, i, a="a", b="b", g="g", x="x", y="y", z="z"):
    ...
    return et(E.est_filtrant_droite(leq, _t(i), x, y, z),
              L.est_systeme_projectif(_t(f), leq, _t(i), a, b, g, x))
...
def est_systeme_image_reciproque(M, u, xp, a="a"):
    ...
    Caractérise la donnée M (M_α := M_indice(M,α) = app("M_indice",M,α)) comme la
...
def systeme_projectif_filtrant_est_systeme(f="f", leq=None, i="I",
    ...
    H = N.assume(est_systeme_projectif_filtrant(_t(f), leq, _t(i), a, b, g, x, y, z))
    return conjonction_elim_droite(H)
...
REPORTES = [
    "Proposition 10 (§III.1.10) : dans un ensemble ordonné filtrant à droite, un "
    "élément maximal est le plus grand — REPORTÉ.",
