...
def _frais(*termes, base="w"):
    ...
    from bourbaki.logique.formule import libres_t, _fraiche
    evite = set()
    for t in termes:
        evite |= libres_t(t)
...
def Theta_reflexive_sur(X="X", G="x"):
    ...
    from bourbaki.ordre.ensembles_ordre_relation import reflexivite_sur
    vX, vG = var(X), var(G)
...
def Theta_transitive(X="X", G="x", H="y", K="z"):
    ...
    if Ht != H:
        _, inner = __import__("bourbaki.logique.tactiques.tactiques_abrege2",
...
def Union_majorant(X="X", D="D", G="x"):
    ...
    from bourbaki.ordre.ensembles_ordre_relation import totalement_ordonne, majorant
    vX, vD = var(X), var(D)
    Ut = Union(vX, vD)
    HDW = N.assume(inclus(vD, W(vX)))                          # 𝔇⊂W
...
def W_inductif(X="X", D="C", m="m", x="x", y="y", z="z"):
    ...
    from bourbaki.ordre.ensembles_zorn import chaine, est_inductif
    from bourbaki.ordre.ensembles_ordre_relation import totalement_ordonne, majorant
    vX, vD = var(X), var(D)
...
def _Ext_bien_ordonne_corps(X, M, x0, GW, Hx0nd, S="S", aa="a", ww="w", s0="s0", m="mw"):
    ...
    if "se" != ww:
        _, inner2 = __import__("bourbaki.logique.tactiques.tactiques_abrege2",
