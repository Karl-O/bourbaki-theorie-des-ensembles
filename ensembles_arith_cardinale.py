...
from formule import (Terme, var, egal, et, impl, appartient, existe,
                     subst_t, subst_f)
...
from ensembles_produit_equipotence import eq_produit_invariant
from ensembles_produit_commute import eq_produit_commute
...
from tactiques_abrege_quantif import (existe_elimination, alpha_existe,
                                      congruence_existe)
from ensembles_cardinaux import equipotent, est_bijection_de
...
def _eq_son_cardinal_terme(vX):
    ...
    from tactiques_abrege2 import instancie
...
def reassoc_graphe_fonctionnel(x="X", y="Y", z="Z"):
    ...
    from ensembles_fonction_terme import graphe_terme_fonctionnel
    A = E.produit(E.produit(_t(x), _t(y)), _t(z))
...
def reassoc_graphe_domaine(x="X", y="Y", z="Z"):
    ...
    from ensembles_cantor import graphe_terme_domaine
    A = E.produit(E.produit(_t(x), _t(y)), _t(z))
...
def reassoc_graphe_valeur(x="X", y="Y", z="Z", u="u"):
    ...
    from ensembles_cantor import graphe_terme_couple_dans
...
def reassoc_graphe_image(x="X", y="Y", z="Z"):
    ...
    from ensembles_produit_commute import (_membre_produit_pr1_ab,
        _membre_produit_pr2_ab, _projection_premiere_ab, _projection_seconde_ab,
        _couple_dans_produit_t, _inst_produit)
    from ensembles_couples import couple_egal_implique_composantes
    from ensembles_fonction_terme import membre_graphe_terme
