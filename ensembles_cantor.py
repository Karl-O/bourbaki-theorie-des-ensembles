...
from formule import (Terme, var, egal, et, non, equiv, appartient,
                     existe, subst_t)
...
from tactiques_abrege_egalite import (symetrie, symetrie as eg_symetrie,
                                      composer_egalites)
from tactiques_abrege_quantif import existe_elimination, congruence_existe, alpha_existe
...
from ensembles_couples import singleton_injectif
from ensembles_produit_famille import membre_parties
from ensembles_cardinaux import est_injection_de
...
def paradoxe_diagonal(p):
    ...
    from tactiques_abrege2 import (conjonction_elim_gauche, conjonction_elim_droite,
                                   contraposition, dni)
    from tactiques_abrege import syllogisme
    H = equiv(p, non(p))                                  # P ⇔ ¬P
    hH = N.assume(H)
...
def est_bijection_de_local(X, F, Y):
    ...
    from ensembles_cardinaux import est_bijection_de as _b
    return _b(F, X, Y)
...
def cantor_non_equipotent(x="X"):
    ...
    from formule import existe as _ex
    vX = _t(x)
    PX = E.parties(vX)
...
def cantor_strict(x="X"):
    ...
    from ensembles_cardinaux import inf_strict_card
