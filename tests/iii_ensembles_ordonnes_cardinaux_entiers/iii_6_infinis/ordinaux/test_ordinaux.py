...
def test_segment_initial_extremite_est_segment_extremite():
    # seg_ext MIGRÉ : le terme porte le GRAPHE G (pas la relation callable R).
    assert O.segment_initial_extremite(Es, G, a) == E.segment_extremite(G, Es, a)
    # et il en DEPEND : un autre graphe donne un AUTRE terme.
    assert O.segment_initial_extremite(Es, G, a) != E.segment_extremite(Gp, Es, a)
