...
def test_temoin_surjectif_dans_S():
    """10 hyps de structure ⊢ (∃pz)((pz∈S et pz∈E) et φ(pz)=u) — la surjectivité
    de l'iso EXTRAITE (bijective→surjective→AXIOME_IMAGE→couple_donne_valeur)."""
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import existe
    th = PV.temoin_surjectif_dans_S()
    vphi, vS, vE, vu = var("phi"), var("S"), var("E"), var("ua")
    vpz = var("pz")
    corps = et(et(appartient(vpz, vS), appartient(vpz, vE)),
               egal(E.valeur(vphi, vpz), vu))
    assert th.conclusion == existe("pz", corps)
    assert len(th.hypotheses) == 10
    assert th.conclusion not in th.hypotheses
    # la surjectivité vient de l'ISO : l'hypothèse iso est au séquent
    Rf, Rpf = _R_de("R"), _R_de("Rp")
    assert V.est_isomorphisme_ordre(vphi, vS, var("T"), Rf, Rpf, "px", "pw") \
        in th.hypotheses
    assert len(E.theorie_ensembles().axiomes) == 22
