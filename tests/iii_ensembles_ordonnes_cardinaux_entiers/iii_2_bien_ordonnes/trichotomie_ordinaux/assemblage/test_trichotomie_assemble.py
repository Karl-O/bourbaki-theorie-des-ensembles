...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.trichotomie_ordinaux.assemblage import ensembles_trichotomie_assemble as TA
...
import bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.trichotomie_ordinaux.maximalite.ensembles_maximalite_close as MAX
...
def test_prouve_maximalite_et_segment_dom_decharges():
    ...
    from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import ou, egal
...
def test_min4_maximalite_dechargee():
    """🎯🎯🎯🎯 Th.3 sous { bo(R,E), bo(Rp,F), h_graphe_hyp } — la MAXIMALITÉ
    DÉRIVÉE (close + les DEUX segments _prouve), val_dans_F/temoin_dans_S mortes."""
    thm = TA.trichotomie_ordinaux_canon_prouve_min4()
    hs = set(thm.hypotheses)
    assert len(hs) == 3
    assert MAX.h_graphe_hyp("E", "R", "F", "Rp") in hs      # l'ULTIME structurelle
    assert not [x for x in hs if x.tag == "ou"]             # maximalité DISPARUE
    assert DS.val_dans_F("E", "R", "F", "Rp") not in hs
    assert thm.conclusion not in hs
    assert len(E.theorie_ensembles().axiomes) == 22
