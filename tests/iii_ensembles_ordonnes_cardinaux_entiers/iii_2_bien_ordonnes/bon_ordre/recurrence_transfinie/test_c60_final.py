"""Tests — §III.2 C60 EXISTENCE, ASSEMBLAGE FINAL (`bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_final`).
...
import bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_final as F
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_coeur import famille_compatible, union_famille
...
def test_extension_un_pas_depuis_coincidence():
    ...
    vD = var("Df")
    U = union_famille(vD)
    seg = E.segment_extremite(var("G"), var("E"), var("x0"))
...
def test_dom_extension_un_pas():
    ...
    assert r.conclusion == egal(E.dom(E.reunion(U, S)), dom_essai(var("G"), var("E"), var("x0")))
...
def test_couvert_essai_depuis_famille():
    ...
    vD = var("Df")
    couvert = couvert_essai(_vh, var("G"), var("E"), "pess", "zess")(var("x0"))
...
def test_heredite_couverture_realisee():
    ...
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_recursion_transfinie_existence import heredite_couverture
    from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie.ensembles_c60_existence_close import couvert_essai
    r = F.heredite_couverture_realisee(_Dfam, _vval, _vh)
    R = _graphe_R("G")
    couvert = couvert_essai(_vh, var("G"), var("E"))
    assert r.conclusion == heredite_couverture(couvert, var("G"), var("E"), "x0tf", "ytf")
    assert len(r.hypotheses) == 1
...
def test_recursion_transfinie_existence():
    ...
    R = _graphe_R("G")
    ve = var("E")
    couvert = couvert_essai(_vh, var("G"), ve)
