...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie import ensembles_recurrence_transfinie as TF
...
def test_enonces_bien_formes():
    e = var("E")
    assert TF.heredite_transfinie(_P, var("G"), e) is not None
    assert TF.conclusion_transfinie(_P, e) is not None
    assert TF.recurrence_transfinie(_P, var("G"), e) is not None
...
def test_conclusion_exacte():
    th = TF.recurrence_transfinie_preuve(_P)
    e = var("E")
    cible = TF.recurrence_transfinie(_P, var("G"), e, "x0tf", "ytf")
...
def test_structure_implication():
    ...
    k = cons.sous[1]
    assert h == TF.heredite_transfinie(_P, var("G"), e, "x0tf", "ytf")
...
def test_c60_unicite_enonces_bien_formes():
    e = var("E")
    assert TF.regle_coherente_sur_segments(_vf, _vg, var("G"), e) is not None
...
def test_c60_unicite_sous_deux_hyps_honnetes():
    ...
    assert TF.regle_coherente_sur_segments(_vf, _vg, var("G"), e, "x0tf", "ytf") in th.hypotheses
