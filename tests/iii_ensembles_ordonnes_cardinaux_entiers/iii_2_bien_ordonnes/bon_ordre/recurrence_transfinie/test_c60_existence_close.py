...
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import var, app, egal, et, non, appartient, pourtout
from bourbaki.ii_theorie_des_ensembles.ii_1_relations_collectivisantes import ensembles_abrege as E
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_2_bien_ordonnes.bon_ordre.recurrence_transfinie import ensembles_c60_existence_close as C
...
def test_e6_couverture_essais_deux_hyps_honnetes():
    ...
    R = _graphe_R("G")
    e = var("E")
    couvert = C.couvert_essai(_vh, var("G"), e)
    assert E.est_bien_ordonne(R, e) in th.hypotheses
    assert heredite_couverture(couvert, var("G"), e, "x0tf", "ytf") in th.hypotheses
...
def test_e6_predicat_essai_bien_forme():
    R = _graphe_R("G")
    e = var("E")
    ess = C.est_essai(var("p"), _vh, var("G"), e, var("x0"))
    # est_essai est une conjonction ((func p et dom p=seg∪{x}) et eq) — bien formée
    assert ess is not None
    assert C.dom_essai(var("G"), e, var("x0")) == E.reunion(
        E.segment_extremite(var("G"), e, var("x0")), E.singleton(var("x0")))
