...
`bon_ordre_intervalle(a)` est DÉRIVÉ de l'UNIQUE hypothèse HONNÊTE, Ro-CLOSE,
`(∃Ro)( bo_form(Ro,a) ∧ (∀c) realisation_segment(Ro,a,c) )` — MIGRATION seg_ext
(2026-07-31) : le segment porte son graphe, la réalisation n'est plus Ro-indépendante,
donc le bon ordre et la propriété demandée passent sous LE MÊME ∃ —, en CONTOURNANT le
liant-TERME dégénéré de
...
"""
from bourbaki.i_description_mathematique_formelle.i_1_termes_relations.outil_formule import var, pourtout, existe, et, libres_f
...
#  portant son graphe, `realisation_hypothese(Ro,a)` a Ro LIBRE ; la laisser en
...
def _hyp_close_main(a="a", Ro="Ro"):
    """(∃Ro)( bo_form(Ro,a) ∧ (∀c) realisation_segment(Ro,a,c) ) — RÉASSEMBLÉE.

    N'appelle PAS `RSP.realisation_hypothese_close` : comparer le module à son propre
    constructeur ne prouverait rien.  On rebâtit ∃/∧ depuis les primitives, sur les
    DEUX énoncés de base (bo_form canonique et la réalisation)."""
    bo = BOIO._bo_form_canon(a, Ro, RSP._TPB, RSP._BM, RSP._BX)
    return existe(Ro, et(bo, RSP.realisation_hypothese(Ro, a)))
...
def test_bon_ordre_intervalle_depuis_realisation_unique_hyp():
    t = RSP.bon_ordre_intervalle_depuis_realisation("a")
    # UNE SEULE hypothèse survivante, comparée au frozenset RÉASSEMBLÉ (pas un len()).
    assert len(t.hypotheses) == 1
    assert frozenset(t.hypotheses) == frozenset({_hyp_close_main("a")})
    # le module doit ANNONCER la même : sa fabrique d'hypothèse ne doit pas dériver.
    assert RSP.bon_ordre_intervalle_depuis_realisation_hypotheses("a") == {_hyp_close_main("a")}
...
def test_bon_ordre_intervalle_depuis_realisation_hyp_est_realisation():
    ...
    `hyp == realisation_hypothese("Ro","a")` PLUS `"Ro" not in libres_f(hyp)` : la
    Ro-indépendance de la réalisation était un ARTEFACT du défaut (le terme de segment
    n'embarquait pas l'ordre).  On épingle maintenant la garantie NOUVELLE."""
    t = RSP.bon_ordre_intervalle_depuis_realisation("a")
    (hyp,) = t.hypotheses
    assert hyp == _hyp_close_main("a")
    # Ro n'est plus libre — non parce qu'il aurait disparu, mais parce qu'il est LIÉ par ∃.
    assert "Ro" not in libres_f(hyp)
    # L'ANCIENNE hypothèse, elle, porte Ro LIBRE : elle ne peut plus jouer ce rôle.
    ancienne = RSP.realisation_hypothese("Ro", "a")
    assert "Ro" in libres_f(ancienne)
    assert hyp != ancienne
...
def test_chaine_cardinaux_bien_ordonnes_sous_realisation():
    ...
    assert frozenset(chained.hypotheses) == frozenset({_hyp_close_main("a")})
    assert len(E.theorie_ensembles().axiomes) == 22
