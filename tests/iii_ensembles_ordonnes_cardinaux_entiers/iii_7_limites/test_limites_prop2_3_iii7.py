...
from bourbaki.iii_ensembles_ordonnes_cardinaux_entiers.iii_7_limites import ensembles_limites_prop2_3_iii7 as P
...
def test_prop2_injectivite_conclusion_y_egal_z():
    ...
    # bonne formation DÉDUITE de « y,z ∈ lim← », plus supposée (point_limite_est_graphe)
    assert E.est_un_graphe(var("yy")) not in th.hypotheses
    assert E.est_un_graphe(var("zz")) not in th.hypotheses
...
def test_prop3_g_injective_pointwise():
    ...
    assert th.conclusion not in th.hypotheses
    assert len(th.hypotheses) == 6   # 8 avant la fusion des deux ecritures de x in lim<-
