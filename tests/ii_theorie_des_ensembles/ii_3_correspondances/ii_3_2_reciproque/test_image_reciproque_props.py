...
def test_inclus_image_reciproque_image_18():
    ...
    # depuis le 8 août 2026 la compagne porte son antécédent H_app (balayage GP10) :
    # la cible EST l'énoncé complet ⊢ H_app ⇒ (X ⊂ f⁻¹⟨f⟨X⟩⟩).
    ...
    assert cible == impl(hyp_app, cible.sous[1])   # l'antécédent est bien H_app
...
def test_image_image_reciproque_inclus_19():
    ...
    # depuis le 8 août 2026 la compagne porte son antécédent est_fonctionnel :
    # la cible EST l'énoncé complet ⊢ est_fonctionnel(f) ⇒ (f⟨f⁻¹⟨Y⟩⟩ ⊂ Y).
    cible = cible_image_image_reciproque_inclus()
    vf = var("f")
    assert cible == impl(E.est_fonctionnel(vf), cible.sous[1])
    assert th.conclusion == cible
    assert th.hypotheses == frozenset()            # clos
    assert len(E.theorie_ensembles().axiomes) == 22
